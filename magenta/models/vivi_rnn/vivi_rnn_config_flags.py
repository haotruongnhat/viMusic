...
"""Provides a class, defaults, and utils for vivi RNN model configuration."""
...
class ViviRnnConfigError(Exception):
  pass


def config_from_flags():
  """Parses flags and returns the appropriate ViviRnnConfig.

  Returns:
    The appropriate ViviRnnConfig based on the supplied flags.

  Raises:
     ViviRnnConfigError: When an invalid config is supplied.
  """
  if FLAGS.config not in vivi_rnn_model.default_configs:
    raise ViviRnnConfigError(
        '`--config` must be one of %s. Got %s.' % (
            vivi_rnn_model.default_configs.keys(), FLAGS.config))
  config = vivi_rnn_model.default_configs[FLAGS.config]
