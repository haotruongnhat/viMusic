...
class ViviRnnSequenceGenerator(sequence_generator.BaseSequenceGenerator):
  ...
  def __init__(self, model, details, steps_per_quarter=4, checkpoint=None,
               bundle=None):
    """Creates an ViviRnnSequenceGenerator.

    Args:
      model: Instance of ViviRnnModel.
    ...
    """
    super(ViviRnnSequenceGenerator, self).__init__(
...
def get_generator_map():
  ...
  def create_sequence_generator(config, **kwargs):
    return ViviRnnSequenceGenerator(
        vivi_rnn_model.ViviRnnModel(config), config.details,
        steps_per_quarter=config.steps_per_quarter, **kwargs)

  return {key: functools.partial(create_sequence_generator, config)
          for (key, config) in vivi_rnn_model.default_configs.items()}
