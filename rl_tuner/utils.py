...
#Read train data
def create_dataset(dataset_name,eval_ratio=0.1):
...
def gen_midi_dataset_by_model(dataset_name,eval_ratio=0.1):
    #default with basic rnn
    config = 'basic_rnn'
    hparams = None
    input_tfrecord = "./datasets/" + dataset_name + ".tfrecord"
    output_tfrecord = './rl_tuner/tmp_dataset/' + dataset_name + '/note_rnn'

    config_model = melody_rnn_model.default_configs[config]
    #config_model.hparams.parse(hparams)

    pipeline_instance = melody_rnn_pipeline.get_pipeline(
      config_model, eval_ratio=eval_ratio)
