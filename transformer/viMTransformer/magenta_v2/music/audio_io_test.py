...
class AudioIoTest(tf.test.TestCase):
  ...
  def setUp(self):
    self.wav_filename = os.path.join(tf.compat.v1.resource_loader.get_data_files_path(),
                                     'testdata/example.wav')
    self.wav_filename_mono = os.path.join(
        tf.compat.v1.resource_loader.get_data_files_path(), 'testdata/example_mono.wav')
