...
class NadeTest(tf.test.TestCase):
  ...
  def testInternalBias(self):
    ...
    num_hidden = 6
    num_dims = 8
    test_inputs = tf.random.normal(shape=(batch_size, num_dims))
  ...
  def testExternalBias(self):
    ...
    num_dims = 8
    test_inputs = tf.random.normal(shape=(batch_size, num_dims))
    test_b_enc = tf.random.normal(shape=(batch_size, num_hidden))
    test_b_dec = tf.random.normal(shape=(batch_size, num_dims))
