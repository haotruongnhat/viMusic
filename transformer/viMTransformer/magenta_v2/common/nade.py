...
def _safe_log(tensor):
  ...
  return tf.math.log(1e-6 + tensor)
...
class Nade(object):
  ...
  def log_prob(self, x, b_enc=None, b_dec=None):
    ...
    x_arr = tf.unstack(
        tf.reshape(tf.transpose(a=x), [self.num_dims, batch_size, 1]))
    w_enc_arr = tf.unstack(self.w_enc)
    w_dec_arr = tf.unstack(self.w_dec_t)
    b_dec_arr = tf.unstack(
        tf.reshape(tf.transpose(a=b_dec), [self.num_dims, batch_size, 1]))
    ...
    return (tf.squeeze(log_p, axis=[1]),
            tf.transpose(a=tf.squeeze(tf.stack(cond_p), [2])))
  ...
  def sample(self, b_enc=None, b_dec=None, n=None, temperature=None):
    ...
    batch_size = n or tf.shape(input=b_enc)[0]
    ...
    def loop_body(i, a, sample, log_p):
      ...
      if temperature is None:
        v_i = tf.cast(tf.greater_equal(cond_p_i, 0.5), dtype=tf.float32)
      else:
    ...
    return (tf.transpose(a=tf.squeeze(tf.stack(sample), [2])),
