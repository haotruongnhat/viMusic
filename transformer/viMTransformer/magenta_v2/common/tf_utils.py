...
def log_loss(labels, predictions, epsilon=1e-7, scope=None, weights=None):
  ...
  """
  with tf.compat.v1.name_scope(scope, "log_loss", (predictions, labels)):
    predictions = tf.cast(predictions, dtype=tf.float32)
    labels = tf.cast(labels, dtype=tf.float32)
    predictions.get_shape().assert_is_compatible_with(labels.get_shape())
    losses = -tf.multiply(labels, tf.math.log(predictions + epsilon)) - tf.multiply(
        (1 - labels), tf.math.log(1 - predictions + epsilon))
