...
def _shuffle_inputs(input_tensors, capacity, min_after_dequeue, num_threads):
  ...
  runner = tf.compat.v1.train.QueueRunner(shuffle_queue, [enqueue_op] * num_threads)
...
def get_padded_batch(file_list, batch_size, input_size, label_shape=None,
  ...
  length = tf.shape(input=sequence['inputs'])[0]
  ...
  tf.compat.v1.logging.info(input_tensors)
...
def count_records(file_list, stop_at=None):
  ...
  for tfrecord_file in file_list:
    tf.compat.v1.logging.info('Counting records in %s.', tfrecord_file)
    for _ in tf.compat.v1.python_io.tf_record_iterator(tfrecord_file):
      num_records += 1
      if stop_at and num_records >= stop_at:
        tf.compat.v1.logging.info('Number of records is at least %d.', num_records)
        return num_records
  tf.compat.v1.logging.info('Total records: %d', num_records)
  return num_records
...
def flatten_maybe_padded_sequences(maybe_padded_sequences, lengths=None):
  ...
  def flatten_padded_sequences():
    indices = tf.compat.v1.where(tf.sequence_mask(lengths))
    return tf.gather_nd(maybe_padded_sequences, indices)

  return tf.cond(
      pred=tf.equal(tf.reduce_min(input_tensor=lengths), tf.shape(input=maybe_padded_sequences)[1]),
      true_fn=flatten_unpadded_sequences,
      false_fn=flatten_padded_sequences)
