...
class ConvertMidiDirToSequencesTest(tf.test.TestCase):
  ...
  def setUp(self):
    midi_filename = os.path.join(tf.compat.v1.resource_loader.get_data_files_path(),
    ...
    tf.io.gfile.mkdir(sub_1_dir)
    tf.io.gfile.mkdir(sub_2_dir)
    tf.io.gfile.mkdir(sub_1_sub_dir)

    tf.io.gfile.copy(midi_filename, os.path.join(root_dir, 'midi_1.mid'))
    tf.io.gfile.copy(midi_filename, os.path.join(root_dir, 'midi_2.mid'))
    tf.io.gfile.copy(midi_filename, os.path.join(sub_1_dir, 'midi_3.mid'))
    tf.io.gfile.copy(midi_filename, os.path.join(sub_2_dir, 'midi_3.mid'))
    tf.io.gfile.copy(midi_filename, os.path.join(sub_2_dir, 'midi_4.mid'))
    tf.io.gfile.copy(midi_filename, os.path.join(sub_1_sub_dir, 'midi_5.mid'))
