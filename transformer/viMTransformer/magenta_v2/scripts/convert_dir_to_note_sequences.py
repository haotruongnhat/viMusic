...
def convert_files(root_dir, sub_dir, writer, recursive=False):
  ...
  tf.compat.v1.logging.info("Converting files in '%s'.", dir_to_convert)
  files_in_dir = tf.io.gfile.listdir(os.path.join(dir_to_convert))
  recurse_sub_dirs = []
  written_count = 0
  for file_in_dir in files_in_dir:
    tf.compat.v1.logging.log_every_n(tf.compat.v1.logging.INFO, '%d files converted.',
                           1000, written_count)
    ...
    if (full_file_path.lower().endswith('.mid') or
      ...
      try:
        ...
        tf.compat.v1.logging.fatal('%r generated an exception: %s', full_file_path, exc)
        continue
      if sequence:
...
def convert_midi(root_dir, sub_dir, full_file_path):
  ...
  tf.compat.v1.logging.info('Converted MIDI file %s.', full_file_path)
  return sequence
...
def convert_musicxml(root_dir, sub_dir, full_file_path):
  ...
  tf.compat.v1.logging.info('Converted MusicXML file %s.', full_file_path)
  return sequence
...
def convert_abc(root_dir, sub_dir, full_file_path):
  ...
  for idx, tune in tunes.iteritems():
    ...
    tf.compat.v1.logging.info('Converted ABC file %s.', full_file_path)
  return sequences
...
def main(unused_argv):
  ...
  if not FLAGS.input_dir:
    tf.compat.v1.logging.fatal('--input_dir required')
    return
  if not FLAGS.output_file:
    tf.compat.v1.logging.fatal('--output_file required')
    return
