...
class Quantizer(NoteSequencePipeline):
  ...
  def transform(self, note_sequence):
    try:
      ...
      tf.compat.v1.logging.warning('Multiple time signatures in NoteSequence %s: %s',
      ...
      tf.compat.v1.logging.warning('Multiple tempos found in NoteSequence %s: %s',
                         note_sequence.filename, e)
      self._set_stats([statistics.Counter(
          'sequences_discarded_because_multiple_tempos', 1)])
      return []
    except sequences_lib.BadTimeSignatureError as e:
      tf.compat.v1.logging.warning('Bad time signature in NoteSequence %s: %s',
                         note_sequence.filename, e)
      self._set_stats([statistics.Counter(
...
class TranspositionPipeline(NoteSequencePipeline):
  ...
  def transform(self, sequence):
    ...
    if sequence.key_signatures:
      tf.compat.v1.logging.warn('Key signatures ignored by TranspositionPipeline.')
    if any(note.pitch_name for note in sequence.notes):
      tf.compat.v1.logging.warn('Pitch names ignored by TranspositionPipeline.')
    if any(ta.annotation_type == CHORD_SYMBOL
           for ta in sequence.text_annotations):
      tf.compat.v1.logging.warn('Chord symbols ignored by TranspositionPipeline.')
