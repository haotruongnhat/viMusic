...
class PipelineTest(tf.test.TestCase):
  ...
  def testFileIteratorRecursive(self):
    ...
    for path, contents in target_files + extra_files:
      ...
      tf.io.gfile.makedirs(os.path.dirname(abs_path))
      tf.io.gfile.GFile(abs_path, mode='w').write(contents)
  ...
  def testRunPipelineSerial(self):
    ...
    self.assertTrue(tf.io.gfile.exists(dataset_1_dir))
    self.assertTrue(tf.io.gfile.exists(dataset_2_dir))

    dataset_1_reader = tf.compat.v1.python_io.tf_record_iterator(dataset_1_dir)
    self.assertEqual(
        set([('serialized:%s_A' % s).encode('utf-8') for s in strings] +
            [('serialized:%s_B' % s).encode('utf-8') for s in strings]),
        set(dataset_1_reader))

    dataset_2_reader = tf.compat.v1.python_io.tf_record_iterator(dataset_2_dir)
    self.assertEqual(
