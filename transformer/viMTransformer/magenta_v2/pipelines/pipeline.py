...
def file_iterator(root_dir, extension=None, recurse=True):
  ...
  dirs = [os.path.join(root_dir, child)
          for child in tf.io.gfile.listdir(root_dir)]
  while dirs:
    sub = dirs.pop()
    if tf.io.gfile.isdir(sub):
      if recurse:
        dirs.extend(
            [os.path.join(sub, child) for child in tf.io.gfile.listdir(sub)])
    else:
...
def tf_record_iterator(tfrecord_file, proto):
  ...
  """
  for raw_bytes in tf.compat.v1.python_io.tf_record_iterator(tfrecord_file):
...
def run_pipeline_serial(pipeline,
  ...
  writers = dict((name, tf.io.TFRecordWriter(path))
  ...
  for input_ in input_iterator:
    ...
    if total_inputs % 500 == 0:
      tf.compat.v1.logging.info('Processed %d inputs so far. Produced %d outputs.',
                      total_inputs, total_outputs)
      statistics.log_statistics_list(stats, tf.compat.v1.logging.info)
  tf.compat.v1.logging.info('\n\nCompleted.\n')
  tf.compat.v1.logging.info('Processed %d inputs total. Produced %d outputs.',
                  total_inputs, total_outputs)
  statistics.log_statistics_list(stats, tf.compat.v1.logging.info)
