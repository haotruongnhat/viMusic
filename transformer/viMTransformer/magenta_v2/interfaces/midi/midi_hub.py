...
try:
  ...
  tf.compat.v1.logging.warn('Could not import RtMidi. Virtual ports are disabled.')
...
class MidiHub(object):
  ...
  def __init__(self, input_midi_ports, output_midi_ports, texture_type,
    ...
    if input_midi_ports:
      ...
      tf.compat.v1.logging.warn('No input port specified. Capture disabled.')
      self._inports = None
  ...
  def _handle_message(self, msg):
    ...
    if msg.type == 'control_change':
      if self._control_values.get(msg.control, None) != msg.value:
        tf.compat.v1.logging.debug('Control change %d: %d', msg.control, msg.value)
