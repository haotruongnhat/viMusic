...
def compute_mse_loss(x, xhat, hparams):
  ...
  with tf.compat.v1.name_scope("Losses"):
    if hparams.raw_audio:
      total_loss = tf.reduce_mean(input_tensor=(x - xhat)**2)
    else:
      # Magnitude
      ...
      fm = utils.frequency_weighted_cost_mask(
          ...
          n_fft=hparams.n_fft)
      mag_loss = tf.reduce_mean(input_tensor=fm * (x[:, :, :, 0] - xhat[:, :, :, 0])**2)
      if hparams.mag_only:
        total_loss = mag_loss
      else:
        # Phase
        if hparams.dphase:
          phase_loss = tf.reduce_mean(input_tensor=fm * m *
                                      (x[:, :, :, 1] - xhat[:, :, :, 1])**2)
        else:
          # Von Mises Distribution "Circular Normal"
          # Added constant to keep positive (Same Probability) range [0, 2]
          phase_loss = 1 - tf.reduce_mean(input_tensor=fm * m * tf.cos(
              (x[:, :, :, 1] - xhat[:, :, :, 1]) * np.pi))
        total_loss = mag_loss + hparams.phase_loss_coeff * phase_loss
        tf.compat.v1.summary.scalar("Loss/Mag", mag_loss)
        tf.compat.v1.summary.scalar("Loss/Phase", phase_loss)
...
def train_op(batch, hparams, config_name):
  ...
  tf.compat.v1.add_to_collection("pitch", batch["pitch"])
  tf.compat.v1.add_to_collection("z", z)
  tf.compat.v1.add_to_collection("xhat", xhat)
  ...
  with tf.compat.v1.name_scope("Optimizer"):
    global_step = tf.compat.v1.get_variable(
        ...
        trainable=False)
    optimizer = tf.compat.v1.train.AdamOptimizer(hparams.learning_rate, hparams.adam_beta)
...
def eval_op(batch, hparams, config_name):
  ...
  # Interpolate
  with tf.compat.v1.name_scope("Interpolation"):
