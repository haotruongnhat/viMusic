...
def save_arrays(savedir, hparams, z_val):
  ...
  with tf.io.gfile.GFile(save_name % "z", "w") as f:
    np.save(f, z_save_val)

  tf.compat.v1.logging.info("Z_Save:{}".format(z_save_val.shape))
  tf.compat.v1.logging.info("Successfully saved to {}".format(save_name % ""))
...
def main(unused_argv):
  ...
  if not tf.io.gfile.exists(savedir):
    ...
    with tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(allow_soft_placement=True)) as sess:
      ...
      try:
        while True:
          ...
          tf.compat.v1.logging.info("Iter: %d" % i)
          tf.compat.v1.logging.info("Z:{}".format(res_val[0].shape))
          i += 1
          if i + 1 % 1 == 0:
