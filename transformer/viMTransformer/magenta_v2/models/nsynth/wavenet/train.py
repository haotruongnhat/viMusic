...
def main(unused_argv=None):
  ...
  logdir = FLAGS.logdir
  tf.compat.v1.logging.info("Saving to %s" % logdir)
  ...
  with tf.Graph().as_default():
    ...
    with tf.device(
        tf.compat.v1.train.replica_device_setter(ps_tasks=FLAGS.ps_tasks,
                                       merge_devices=True)):
      ...
      for key, value in config.learning_rate_schedule.iteritems():
        lr = tf.cond(
            pred=tf.less(global_step, key), true_fn=lambda: lr, false_fn=lambda: tf.constant(value))
      ...
      opt = tf.compat.v1.train.SyncReplicasOptimizer(
          tf.compat.v1.train.AdamOptimizer(lr, epsilon=1e-8),
          worker_replicas,
