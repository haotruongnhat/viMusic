...
def shift_right(x):
  ...
  x_padded = tf.pad(tensor=x, paddings=[[0, 0], [1, 0], [0, 0]])
...
def time_to_batch(x, block_size):
  ...
  y = tf.transpose(a=y, perm=[0, 2, 1, 3])
  y = tf.reshape(y, [
...
def conv1d(x,
           ...
           dilation=1,
           causal=True,
           kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, distribution="uniform"),
  ...
  if filter_length > 1 and causal:
    x_ttb = tf.pad(tensor=x_ttb, paddings=[[0, 0], [filter_length - 1, 0], [0, 0]])

  x_ttb_shape = x_ttb.get_shape().as_list()
  x_4d = tf.reshape(x_ttb, [x_ttb_shape[0], 1,
                            x_ttb_shape[1], num_input_channels])
  y = tf.nn.conv2d(input=x_4d, filters=weights, strides=strides, padding=padding)
