...
def main(unused_argv=None):
  ...
  if FLAGS.checkpoint_path:
    ...
    tf.compat.v1.logging.info("Will load latest checkpoint from %s.", expdir)
    while not tf.io.gfile.exists(expdir):
      tf.compat.v1.logging.fatal("\tExperiment save dir '%s' does not exist!", expdir)
      sys.exit(1)

    try:
      checkpoint_path = tf.train.latest_checkpoint(expdir)
    except tf.errors.NotFoundError:
      tf.compat.v1.logging.fatal("There was a problem determining the latest checkpoint.")
      sys.exit(1)

  if not tf.compat.v1.train.checkpoint_exists(checkpoint_path):
    tf.compat.v1.logging.fatal("Invalid checkpoint path: %s", checkpoint_path)
    sys.exit(1)

  tf.compat.v1.logging.info("Will restore from checkpoint: %s", checkpoint_path)

  source_path = utils.shell_path(FLAGS.source_path)
  tf.compat.v1.logging.info("Will load Wavs from %s." % source_path)

  save_path = utils.shell_path(FLAGS.save_path)
  tf.compat.v1.logging.info("Will save embeddings to %s." % save_path)
  if not tf.io.gfile.exists(save_path):
    tf.compat.v1.logging.info("Creating save directory...")
  ...
  wavfiles = sorted([
      ...
      for fname in tf.io.gfile.listdir(source_path) if is_wav(fname)
  ])

  for start_file in xrange(0, len(wavfiles), batch_size):
    batch_number = (start_file / batch_size) + 1
    tf.compat.v1.logging.info("On file number %s (batch %d).", start_file, batch_number)
    ...
    try:
      ...
      tf.compat.v1.logging.info(encoding.shape)
      tf.compat.v1.logging.info("Sample length: %d" % sample_length)

      for num, (wavfile, enc) in enumerate(zip(wavefiles_batch, encoding)):
        filename = "%s_embeddings.npy" % wavfile.split("/")[-1].strip(".wav")
        with tf.io.gfile.GFile(os.path.join(save_path, filename), "w") as f:
          np.save(f, enc)

        if num + batch_filler + 1 == batch_size:
          break
    except Exception as e:
      tf.compat.v1.logging.info("Unexpected error happened: %s.", e)
      raise
