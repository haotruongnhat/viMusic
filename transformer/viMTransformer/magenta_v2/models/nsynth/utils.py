...
def mu_law(x, mu=255, int8=False):
  ...
  """
  out = tf.sign(x) * tf.math.log(1 + mu * tf.abs(x)) / np.log(1 + mu)
...
def inv_mu_law(x, mu=255):
  ...
  out = tf.compat.v1.where(tf.equal(x, 0), x, out)
  return out
...
def tf_specgram(audio,
  ...
  return tf.compat.v1.py_func(batch_specgram, [
...
def tf_ispecgram(spec,
  ...
  if pad:
    ...
    x = spec
  audio = tf.compat.v1.py_func(batch_ispecgram, [
...
def specgram_summaries(spec,
  ...
  if hparams.raw_audio:
    ...
    spec /= tf.expand_dims(tf.reduce_max(input_tensor=spec, axis=1), axis=1)
    ...
    if image:
      if b % columns != 0:
        ...
        rows = b / columns
      tf.compat.v1.summary.image("Mag/%s" % name,
                       form_image_grid(spec[:b, :, :, :1], [rows, columns],
                                       [n_freq, n_time], 1))
      if phase:
        tf.compat.v1.summary.image("Phase/%s" % name,
                         form_image_grid(spec[:b, :, :, 1:], [rows, columns],
                                         [n_freq, n_time], 1))
    if audio:
      tf.compat.v1.summary.audio(
...
def calculate_softmax_and_summaries(logits, one_hot_labels, name):
  ...
  loss = tf.nn.softmax_cross_entropy_with_logits(
      logits=logits, labels=tf.stop_gradient(one_hot_labels))
...
def softmax_summaries(loss, logits, one_hot_labels, name="softmax"):
  ...
  """
  tf.compat.v1.summary.scalar(name + "_loss", loss)

  one_hot_labels = tf.cond(
      pred=tf.equal(tf.rank(one_hot_labels),
               2), true_fn=lambda: tf.cast(tf.argmax(input=one_hot_labels, axis=1), dtype=tf.int32),
      false_fn=lambda: tf.cast(one_hot_labels, dtype=tf.int32))

  in_top_1 = tf.nn.in_top_k(predictions=logits, targets=one_hot_labels, k=1)
  tf.compat.v1.summary.scalar(name + "_precision@1",
                    tf.reduce_mean(input_tensor=tf.cast(in_top_1, dtype=tf.float32)))
  in_top_5 = tf.nn.in_top_k(predictions=logits, targets=one_hot_labels, k=5)
  tf.compat.v1.summary.scalar(name + "_precision@5",
                    tf.reduce_mean(input_tensor=tf.cast(in_top_5, dtype=tf.float32)))
...
def calculate_l2_and_summaries(predicted_vectors, true_vectors, name):
  ...
  """
  loss = tf.reduce_mean(input_tensor=(predicted_vectors - true_vectors)**2)
  tf.compat.v1.summary.scalar(name + "_loss", loss)
  tf.compat.v1.summary.scalar(
      name + "_prediction_mean_squared_norm",
      tf.reduce_mean(input_tensor=tf.nn.l2_loss(predicted_vectors)))
  tf.compat.v1.summary.scalar(
      name + "_label_mean_squared_norm",
      tf.reduce_mean(input_tensor=tf.nn.l2_loss(true_vectors)))
  return loss
...
def pitch_embeddings(batch,
  ...
  with tf.compat.v1.variable_scope("PitchEmbedding", reuse=reuse):
...
def conv2d(x,
  ...
  with tf.compat.v1.variable_scope(scope + "_Layer"):
    ...
    for layer_idx in range(stacked_layers):
      with slim.arg_scope(
        ...
        if gated:
          with tf.compat.v1.variable_scope("Gated"):
...
def causal_linear(x, n_inputs, n_outputs, name, filter_length, rate,
  ...
  # create queue
  q_1 = tf.queue.FIFOQueue(rate, dtypes=tf.float32, shapes=(batch_size, 1, n_inputs))
  q_2 = tf.queue.FIFOQueue(rate, dtypes=tf.float32, shapes=(batch_size, 1, n_inputs))
