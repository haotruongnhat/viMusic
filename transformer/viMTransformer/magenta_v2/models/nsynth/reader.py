...
class NSynthDataset(object):
  ...
  def get_example(self, batch_size):
    ...
    capacity = batch_size
    path_queue = tf.compat.v1.train.input_producer(
        [self.record_path],
  ...
  def get_wavenet_batch(self, batch_size, length=64000):
    ...
    if self.is_training:
      ...
      key, crop, pitch = tf.compat.v1.train.shuffle_batch(
          [key, crop, pitch],
          batch_size,
          num_threads=4,
  ...
  def get_baseline_batch(self, hparams):
    ...
    if hop_length and n_fft:
      ...
      tf.compat.v1.logging.info("SPECGRAM BEFORE PADDING", specgram)

      if hparams.pad:
        # Pad and crop specgram to 256x256
        num_padding = 2**int(np.ceil(np.log(shape[2]) / np.log(2))) - shape[2]
        tf.compat.v1.logging.info("num_pading: %d" % num_padding)
        specgram = tf.reshape(specgram, shape)
        specgram = tf.pad(tensor=specgram, paddings=[[0, 0], [0, 0], [0, num_padding], [0, 0]])
        specgram = tf.slice(specgram, [0, 0, 0, 0], [-1, shape[1] - 1, -1, -1])
        tf.compat.v1.logging.info("SPECGRAM AFTER PADDING", specgram)

    # Form a Batch
    if self.is_training:
      (audio, velocity, pitch, specgram,
       instrument_source, instrument_family,
       qualities) = tf.compat.v1.train.shuffle_batch(
           [
               audio, velocity, pitch, specgram,
      ...
      (audio, velocity, pitch, specgram,
       instrument_source, instrument_family, qualities) = tf.compat.v1.train.batch(
           [
               audio, velocity, pitch, specgram,
               instrument_source, instrument_family, qualities
           ],
