...
def load_hparams(checkpoint_path):
  ...
  with tf.io.gfile.GFile(hparams_fpath, 'r') as p:
...
class Hyperparameters(object):
  ...
  def __init__(self, *unused_args, **init_hparams):
    ...
    """
    tf.compat.v1.logging.info('Instantiating hparams...')
...
class Straight(Architecture):
  ...
  def __init__(self, input_depth, num_layers, num_filters, num_pitches,  # pylint:disable=unused-argument
               output_depth, **kwargs):
    tf.compat.v1.logging.info('model_type=%s, input_depth=%d, output_depth=%d',
    ...
    tf.compat.v1.logging.info('num_layers=%d, num_filters=%d',
...
class Dilated(Architecture):
  ...
  def __init__(self, input_depth, num_layers, num_filters, num_pitches,  # pylint:disable=unused-argument
    ...
    if kwargs['repeat_last_dilation_level']:
      tf.compat.v1.logging.info('Increasing max dilation level from %s to %s',
    ...
    for _ in range(num_dilation_blocks):
      for level in range(max_dilation_level + 1):
        ...
        tf.compat.v1.logging.info('layer_dilation_rates %r', layer_dilation_rates)
        ...
        tf.compat.v1.logging.info('num_split %d', current_num_pointwise_splits)
        _add(filters=[3, 3, num_filters, num_filters],
