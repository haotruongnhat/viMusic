...
class CoconetGraph(object):
  ...
  def build(self):
    ...
    for var in tf.compat.v1.trainable_variables():
      tf.compat.v1.logging.info('%s_%r', var.name, var.get_shape().as_list())
  ...
  def setup_optimizer(self):
    ...
    self.decay_op = tf.compat.v1.assign(self.learning_rate, 0.5 * self.learning_rate)
  ...
  def compute_cross_entropy(self, logits, labels):
    if self.hparams.use_softmax_loss:
      ...
      # remain constant
      return -tf.nn.log_softmax(logits, axis=2) * labels
    else:
  ...
  def compute_loss(self, unreduced_loss):
    ...
    batch_duration = tf.shape(input=self.pianorolls)[1]
    indices = tf.cast(tf.range(batch_duration), dtype=tf.float32)
    pad_mask = tf.cast(
        indices[None, :, None, None] < self.lengths[:, None, None, None], dtype=tf.float32)
    ...
    dd = (
        self.lengths[:, None, None, None] * tf.cast(
            tf.shape(input=self.pianorolls)[variable_axis], dtype=tf.float32))
    reduced_dd = tf.reduce_sum(input_tensor=dd)

    # Compute numbers of variables to be predicted/conditioned on
    mask_size = tf.reduce_sum(input_tensor=mask, axis=[1, variable_axis], keepdims=True)
    unmask_size = tf.reduce_sum(input_tensor=unmask, axis=[1, variable_axis], keepdims=True)
    ...
    self.loss_total = tf.reduce_sum(input_tensor=unreduced_loss) / reduced_dd
    ...
    self.reduced_mask_size = tf.reduce_sum(input_tensor=mask_size[:, :, 0, :])
    self.reduced_unmask_size = tf.reduce_sum(input_tensor=unmask_size[:, :, 0, :])

    assert_partition_op = tf.group(
        tf.compat.v1.assert_equal(tf.reduce_sum(input_tensor=mask * unmask), 0.),
        tf.compat.v1.assert_equal(self.reduced_mask_size + self.reduced_unmask_size,
                        reduced_dd))
    with tf.control_dependencies([assert_partition_op]):
      self.loss_mask = (
          tf.reduce_sum(input_tensor=mask * unreduced_loss) / self.reduced_mask_size)
      self.loss_unmask = (
          tf.reduce_sum(input_tensor=unmask * unreduced_loss) / self.reduced_unmask_size)
  ...
  def apply_convolution(self, x, layer, layer_idx):
    ...
    fanin = tf.cast(tf.reduce_prod(input_tensor=filter_shape[:-1]), dtype=tf.float32)
    stddev = tf.sqrt(tf.compat.v1.div(2.0, fanin))
    ...
    if regular_convs:
      ...
      tf.compat.v1.logging.info('num_splits %d', num_splits)
      if num_splits > 1:
        num_outputs = None
      ...
      if num_splits > 1:
        ...
        pointwise_splits = [
            tf.compat.v1.layers.dense(splits[i], filter_shape[3]/num_splits,
  ...
  def apply_batchnorm(self, x):
    ...
    gammas = tf.compat.v1.get_variable(
        'gamma', [1, 1, 1, output_dim],
        initializer=tf.compat.v1.constant_initializer(0.1))
    betas = tf.compat.v1.get_variable(
        'beta', [output_dim], initializer=tf.compat.v1.constant_initializer(0.))

    popmean = tf.compat.v1.get_variable(
        'popmean',
        ...
        initializer=tf.compat.v1.constant_initializer(0.0))
    popvariance = tf.compat.v1.get_variable(
        'popvariance',
        shape=[1, 1, 1, output_dim],
        trainable=False,
        collections=[
    ...
    decay = 0.01
    if self.is_training:
      batchmean, batchvariance = tf.nn.moments(x=x, axes=[0, 1, 2], keepdims=True)
...
def get_placeholders(hparams):
  return dict(
      ...
      masks=tf.compat.v1.placeholder(
          tf.float32,
          [None, None, hparams.num_pitches, hparams.num_instruments]),
      lengths=tf.compat.v1.placeholder(tf.float32, [None]))
...
def build_graph(is_training,
  ...
  initializer = tf.compat.v1.random_uniform_initializer(-hparams.init_scale,
                                              hparams.init_scale)
...
def load_checkpoint(path, instantiate_sess=True):
  ...
  tf.compat.v1.logging.info('Loading checkpoint from %s', path)
  ...
  with wmodel.graph.as_default():
    wmodel.sess = tf.compat.v1.Session()
    saver = tf.compat.v1.train.Saver()
    tf.compat.v1.logging.info('loading checkpoint %s', path)
