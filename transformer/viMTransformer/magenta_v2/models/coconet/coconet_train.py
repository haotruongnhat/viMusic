...
def run_epoch(supervisor, sess, m, dataset, hparams, eval_op, experiment_type,
  ...
  if FLAGS.log_progress:
    summaries = tf.compat.v1.Summary()
...
def main(unused_argv):
  ...
  if FLAGS.data_dir is None:
    tf.compat.v1.logging.fatal('No input directory was provided.')
  ...
  tf.compat.v1.logging.info('Writing to %s', config_fpath)
  ...
  with tf.Graph().as_default():
    ...
    sv = tf.compat.v1.train.Supervisor(
        logdir=logdir,
        saver=tf.compat.v1.train.Supervisor.USE_DEFAULT if FLAGS.log_progress else None,
        summary_op=None,
...
class Tracker(object):
  ...
  def __call__(self, loss, sess):
    if self.sign * loss > self.sign * self.best:
      if FLAGS.log_progress:
        tf.compat.v1.logging.info('Previous best %s: %.4f.', self.label, self.best)
        tf.io.gfile.makedirs(os.path.dirname(self.save_path))
        self.saver.save(sess, self.save_path)
        tf.compat.v1.logging.info('Storing best model so far with loss %.4f at %s.' %
                        (loss, self.save_path))
      self.best = loss
      self.age = 0
      self.true_age = 0
