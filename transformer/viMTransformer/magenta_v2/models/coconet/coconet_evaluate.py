...
def main(unused_argv):
  ...
  if not checkpoint_dir:
    ...
    possible_checkpoint_dirs = tf.io.gfile.listdir(FLAGS.eval_logdir)
    possible_checkpoint_dirs = [
        i for i in possible_checkpoint_dirs if
        tf.io.gfile.isdir(os.path.join(FLAGS.eval_logdir, i))]
    ...
    if len(possible_checkpoint_dirs) == 1:
      ...
      tf.compat.v1.logging.info('Using checkpoint dir: %s', checkpoint_dir)
    else:
      raise ValueError(
...
def evaluate_fold(fold, evaluator, hparams, eval_logdir, checkpoint_dir):
  ...
  tf.compat.v1.logging.info('Writing to path: %s' % log_fpath)
...
def evaluate_paths(paths, evaluator, unused_hparams, eval_logdir):
  ...
  for path in paths:
    ...
    rval = lib_evaluation.evaluate(evaluator, pianorolls)
    tf.compat.v1.logging.info('Writing evaluation statistics to %s', log_fpath)
    with lib_util.atomic_file(log_fpath) as p:
      np.savez_compressed(p, **rval)
...
def get_fold_pianorolls(fold, hparams):
  ...
  tf.compat.v1.logging.info('Retrieving pianorolls from %s set of %s dataset.',
...
def get_path_pianorolls(path):
  pianoroll_fpath = os.path.join(tf.compat.v1.resource_loader.get_data_files_path(), path)
  tf.compat.v1.logging.info('Retrieving pianorolls from %s', pianoroll_fpath)
  with tf.io.gfile.GFile(pianoroll_fpath, 'r') as p:
    pianorolls = np.load(p)
  if isinstance(pianorolls, np.ndarray):
    tf.compat.v1.logging.info(pianorolls.shape)
  print_statistics(pianorolls)
  return pianorolls
...
def print_statistics(pianorolls):
  ...
  if isinstance(pianorolls, np.ndarray):
    tf.compat.v1.logging.info(pianorolls.shape)
  tf.compat.v1.logging.info('# of total pieces in set: %d', len(pianorolls))
  lengths = [len(roll) for roll in pianorolls]
  if len(np.unique(lengths)) > 1:
    tf.compat.v1.logging.info('lengths %s', np.sort(lengths))
  tf.compat.v1.logging.info('max_len %d', max(lengths))
  tf.compat.v1.logging.info(
      'unique lengths %s',
      np.unique(sorted(pianoroll.shape[0] for pianoroll in pianorolls)))
  tf.compat.v1.logging.info('shape %s', pianorolls[0].shape)
