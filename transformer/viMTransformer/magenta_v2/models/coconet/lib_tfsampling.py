...
class CoconetSampleGraph(object):
  ...
  def get_placeholders(self):
    ...
    return dict(
        pianorolls=tf.compat.v1.placeholder(
            tf.bool,
        ...
        outer_masks=tf.compat.v1.placeholder_with_default(
            np.zeros(
                (1, 1, hparams.num_pitches, hparams.num_instruments),
                dtype=np.float32),
            [None, None, hparams.num_pitches, hparams.num_instruments],
            "outer_masks"),
        sample_steps=tf.compat.v1.placeholder_with_default(0, (), "sample_steps"),
        total_gibbs_steps=tf.compat.v1.placeholder_with_default(
            0, (), "total_gibbs_steps"),
        current_step=tf.compat.v1.placeholder_with_default(0, (), "current_step"),
        temperature=tf.compat.v1.placeholder_with_default(0.99, (), "temperature"))
  ...
  def make_outer_masks(self, outer_masks, input_pianorolls):
    ...
    outer_masks = tf.cast(outer_masks, dtype=tf.float32)
    ...
    outer_masks = tf.cond(
        pred=tf.reduce_all(input_tensor=tf.equal(outer_masks, 0)),
        true_fn=lambda: make_completion_masks(input_pianorolls),
        false_fn=lambda: outer_masks)
    return outer_masks
  ...
  def build_sample_graph(self, input_pianorolls=None, outer_masks=None,
    ...
    tt = tf.shape(input=input_pianorolls)[1]
    sample_steps = tf.cast(self.inputs["sample_steps"], dtype=tf.float32)
    if total_gibbs_steps is None:
      total_gibbs_steps = self.inputs["total_gibbs_steps"]
    temperature = self.inputs["temperature"]

    input_pianorolls = tf.cast(input_pianorolls, dtype=tf.float32)
    ...
    total_gibbs_steps = tf.cond(
        pred=tf.equal(total_gibbs_steps, 0),
        true_fn=lambda: tf.cast(tt * self.hparams.num_instruments, dtype=tf.float32),
        false_fn=lambda: tf.cast(total_gibbs_steps, dtype=tf.float32))

    # sample_steps is set to total_gibbs_steps if not given.
    sample_steps = tf.cond(
        pred=tf.equal(sample_steps, 0),
        true_fn=lambda: total_gibbs_steps,
        false_fn=lambda: tf.cast(sample_steps, dtype=tf.float32))
    ...
    def infer_step(pianorolls, step_count):
      ...
      masks = make_bernoulli_masks(tf.shape(input=pianorolls), mask_prob,
                                   outer_masks)
      ...
      check_completion_op = tf.compat.v1.assert_equal(
          tf.compat.v1.where(tf.equal(tf.reduce_max(input_tensor=masks, axis=2), 1.),
                   tf.reduce_max(input_tensor=outputs, axis=2),
                   tf.reduce_max(input_tensor=pianorolls, axis=2)),
          1.)
    ...
    current_step = tf.cast(self.inputs["current_step"], dtype=tf.float32)

    # Initializes pianorolls by evaluating the model once to fill in all gaps.
    logits = self.predict(tf.cast(input_pianorolls, dtype=tf.float32), outer_masks)
    samples = sample_with_temperature(logits, temperature=temperature)
    tf.compat.v1.get_variable_scope().reuse_variables()

    self.samples, current_step = tf.while_loop(
        cond=lambda samples, current_step: current_step < sample_steps,
        body=infer_step, loop_vars=[samples, current_step],
        shape_invariants=[
  ...
  def predict(self, pianorolls, masks):
    ...
    direct_inputs = dict(
        ...
        lengths=tf.cast([tf.shape(input=pianorolls)[1]], dtype=tf.float32))
  ...
  def instantiate_sess_and_restore_checkpoint(self):
    ...
    tf.compat.v1.logging.info("loading checkpoint %s", chkpt_fpath)
  ...
  def run(self,
    ...
    run_options = None
    if timeout_ms:
      run_options = tf.compat.v1.RunOptions(timeout_in_ms=timeout_ms)
    ...
    tf.compat.v1.logging.info("exit  %s (%.3fmin)" % (label, time_taken))
    return dict(pianorolls=new_piece, time_taken=time_taken)


def make_completion_masks(pianorolls, outer_masks=1.):
  pianorolls = tf.cast(pianorolls, dtype=tf.float32)
  masks = tf.reduce_all(input_tensor=tf.equal(pianorolls, 0), axis=2, keepdims=True)
  inner_masks = tf.cast(masks, dtype=tf.float32) + 0 * pianorolls
...
def make_bernoulli_masks(shape, pm, outer_masks=1.):
  ...
  pp = shape[2]
  ii = shape[3]
  probs = tf.random.uniform([bb, tt, ii])
  masks = tf.tile(tf.cast(tf.less(probs, pm), dtype=tf.float32)[:, :, None, :], [1, 1, pp, 1])
...
def sample_with_temperature(logits, temperature):
  ...
  pitch_range = tf.shape(input=logits)[-1]

  def sample_from_logits(logits):
    with tf.control_dependencies([tf.compat.v1.assert_greater(temperature, 0.0)]):
      logits = tf.identity(logits)
    reshaped_logits = (
        tf.reshape(logits, [-1, tf.shape(input=logits)[-1]]) / temperature)
    choices = tf.random.categorical(logits=reshaped_logits, num_samples=1)
    choices = tf.reshape(choices,
                         tf.shape(input=logits)[:logits.get_shape().ndims - 1])
    return choices

  choices = tf.cond(pred=tf.equal(temperature, 0.0),
                    true_fn=lambda: tf.argmax(input=tf.nn.softmax(logits), axis=-1),
                    false_fn=lambda: sample_from_logits(logits))
  samples_onehot = tf.one_hot(choices, pitch_range)
  return tf.transpose(a=samples_onehot, perm=[0, 1, 3, 2])
...
def main(unused_argv):
  ...
  tf.compat.v1.logging.info("num of notes in piece %d", np.sum(generated_piece))
