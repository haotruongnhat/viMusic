...
class PianorollEncoderDecoder(object):
  ...
  def encode_midi_melody_to_pianoroll(self, midi):
    ...
    tf.compat.v1.logging.debug('Roll shape: %s', roll.shape)
    roll = roll.T
    tf.compat.v1.logging.debug('Roll argmax: %s', np.argmax(roll, 1))
    return roll
  ...
  def encode_midi_to_pianoroll(self, midi, requested_shape):
    ...
    tf.compat.v1.logging.debug('# of instr %d', len(midi.instruments))
    ...
    tf.compat.v1.logging.debug('Requested roll shape: %s', requested_shape)
    tf.compat.v1.logging.debug('Roll argmax: %s',
