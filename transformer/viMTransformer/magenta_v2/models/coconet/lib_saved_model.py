...
def get_signature_def(model, use_tf_sampling):
  ...
  if use_tf_sampling:
    return tf.compat.v1.saved_model.signature_def_utils.predict_signature_def(
        inputs={
...
def export_saved_model(model, destination, tags, use_tf_sampling):
  ...
  if model is None or destination is None or not destination:
    tf.compat.v1.logging.error('No model or destination provided.')
    return

  builder = tf.compat.v1.saved_model.builder.SavedModelBuilder(destination)
