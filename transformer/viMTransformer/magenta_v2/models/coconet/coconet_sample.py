...
def main(unused_argv):
  ...
  tf.compat.v1.logging.info("basepath: %s", basepath)
  tf.io.gfile.makedirs(basepath)

  # Saves the results as midi or returns as midi out.
  midi_path = os.path.join(basepath, "midi")
  tf.io.gfile.makedirs(midi_path)
  tf.compat.v1.logging.info("Made directory %s", midi_path)
  save_midis(midi_outs, midi_path, label)

  result_npy_save_path = os.path.join(basepath, "generated_result.npy")
  tf.compat.v1.logging.info("Writing final result to %s", result_npy_save_path)
  with tf.io.gfile.GFile(result_npy_save_path, "w") as p:
  ...
  with lib_util.timing("writing_out_sample_npz"):
    tf.compat.v1.logging.info("Writing intermediate steps to %s", intermediate_steps_path)
  ...
  tf.compat.v1.logging.info("Reading to check %s", intermediate_steps_path)
  with tf.io.gfile.GFile(intermediate_steps_path, "r") as p:
    foo = np.load(p)
    for key in foo.keys():
      if re.match(r"0_root/.*?_strategy/.*?_context/0_pianorolls", key):
        ...
        tf.compat.v1.logging.info("Writing context to %s", context_fpath)
...
class Generator(object):
  ...
  def run_generation(self,
    ...
    tf.compat.v1.logging.info("Tentative shape of pianorolls to be generated: %r", shape)
...
class TFGenerator(object):
  ...
  def run_generation(self,
    ...
    tf.compat.v1.logging.info("output pianorolls shape: %r", self.pianorolls.shape)
...
def get_midi_from_pianorolls(rolls, decoder):
  ...
  for pianoroll in rolls:
    tf.compat.v1.logging.info("pianoroll shape: %r", pianoroll.shape)
...
def save_midis(midi_datas, midi_path, label=""):
  for i, midi_data in enumerate(midi_datas):
    ...
    tf.compat.v1.logging.info("Writing midi to %s", midi_fpath)
