...
class Model(object):
  ...
  def load_from_path(cls, path, flags=None):
    ...
    try:
      # Read json to dict
      with tf.io.gfile.GFile(experiment_json_path, 'r') as f:
    ...
    train_sub_dirs = sorted([sub_dir for sub_dir in tf.io.gfile.listdir(path)
  ...
  def __init__(self, stage_id, batch_size, config):
    ...
    tf.compat.v1.summary.scalar('train_time', train_time)
    ...
    def _compute_ac_loss(images, target_one_hot_labels):
      with tf.compat.v1.variable_scope(gan_model.discriminator_scope, reuse=True):
    ...
    def _compute_gl_consistency_loss(data):
      ...
      get_max = lambda x: tf.reduce_max(input_tensor=x, axis=(1, 2), keepdims=True)
      mag_max = get_max(mag)
      new_mag_max = get_max(new_mag)
      mag_scale = tf.maximum(1.0, tf.maximum(mag_max, new_mag_max))
      mag_diff = (mag - new_mag) / mag_scale
      mag_loss = tf.reduce_mean(input_tensor=tf.square(mag_diff))
      return mag_loss
    ...
    with tf.compat.v1.name_scope('losses'):
      ...
      tf.compat.v1.summary.scalar('fake_ac_loss', fake_ac_loss)
      tf.compat.v1.summary.scalar('real_ac_loss', real_ac_loss)
      tf.compat.v1.summary.scalar('wx_fake_ac_loss', wx_fake_ac_loss)
      tf.compat.v1.summary.scalar('wx_real_ac_loss', wx_real_ac_loss)
      tf.compat.v1.summary.scalar('total_gen_loss', gan_loss.generator_loss)
      tf.compat.v1.summary.scalar('total_dis_loss', gan_loss.discriminator_loss)

      if is_fourier:
        tf.compat.v1.summary.scalar('fake_gl_loss', fake_gl_loss)
        tf.compat.v1.summary.scalar('real_gl_loss', real_gl_loss)
        tf.compat.v1.summary.scalar('wx_fake_gl_loss', wx_fake_gl_loss)
    ...
    gan_train_ops, generator_vars_to_restore = \
        train_util.add_generator_smoothing_ops(generator_ema,
                                               gan_model,
                                               gan_train_ops)
    load_scope = tf.compat.v1.variable_scope(
    ...
    labels_ph = tf.compat.v1.placeholder(tf.int32, [batch_size])
    noises_ph = tf.compat.v1.placeholder(tf.float32, [batch_size,
  ...
  def add_summaries(self):
    ...
    def _add_waves_summary(name, waves, max_outputs):
      tf.compat.v1.summary.audio(name,
                       waves,
