...
class DataNormalizer(object):
  ...
  def exists(self):
    return tf.io.gfile.exists(self._done_path)
  ...
  def save(self, data):
    ...
    if not tf.io.gfile.exists(self._work_dir):
      tf.io.gfile.makedirs(self._work_dir)
    with tf.io.gfile.GFile(self._path, 'wb') as f:
      f.write(bytes_io.getvalue())
    with tf.io.gfile.GFile(self._done_path, 'w') as f:
      f.write('')
    return normalizer
  ...
  def load(self):
    ...
    with tf.io.gfile.GFile(self._path, 'rb') as f:
      result = np.load(f)
