...
def get_stage_ids(**kwargs):
  ...
  """
  train_sub_dirs = [
      sub_dir for sub_dir in tf.io.gfile.listdir(kwargs['train_root_dir'])
...
def make_interpolated_latent_vectors(num_rows, num_columns, **kwargs):
  ...
  for _ in range(num_rows):
    ...
    r = tf.reshape(
        tf.cast(tf.range(num_columns), dtype=tf.float32) / (num_columns - 1), [-1, 1])
    dz = z[1] - z[0]
...
def define_train_ops(gan_model, gan_loss, **kwargs):
  ...
  with tf.compat.v1.variable_scope('progressive_gan_train_ops') as var_scope:
    ...
    gen_opt = tf.compat.v1.train.AdamOptimizer(kwargs['generator_learning_rate'], beta1,
                                     beta2)
    dis_opt = tf.compat.v1.train.AdamOptimizer(kwargs['discriminator_learning_rate'],
                                     beta1, beta2)
...
def make_scaffold(stage_id, optimizer_var_list, **kwargs):
  ...
  if stage_id > 0 and curr_ckpt is None:
    ...
    # need to be restored.
    restore_var_list = [
        var for var in tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES)
...
class ProganDebugHook(tf.estimator.SessionRunHook):
  """Prints summary statistics of all tf variables."""

  def __init__(self):
    super(ProganDebugHook, self).__init__()
    self._fetches = [v for v in tf.compat.v1.global_variables()]
...
class TrainTimeHook(tf.estimator.SessionRunHook):
  ...
  def __init__(self, train_time, time_limit=None):
    ...
    self._increment_amount = tf.compat.v1.placeholder(tf.float32, None)
    self._increment_op = tf.compat.v1.assign_add(train_time, self._increment_amount)
  ...
  def before_run(self, _):
    ...
    if self._last_run_duration is not None:
      ...
      return tf.estimator.SessionRunArgs([self._train_time])
...
def train(model, **kwargs):
  ...
  if model.stage_train_time_limit is None:
    hooks.append(tf.estimator.StopAtStepHook(last_step=model.num_images))
  hooks.append(tf.estimator.LoggingTensorHook(
