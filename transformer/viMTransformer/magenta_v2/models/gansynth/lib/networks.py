...
def compute_progress(current_image_id, stable_stage_num_images,
  ...
  progress_fraction = tf.maximum(
      ...
      tf.cast(
          ...
          stable_stage_num_images, dtype=tf.float32) / tf.cast(transition_stage_num_images, dtype=tf.float32))
  ...
  with tf.compat.v1.variable_scope(scope, reuse=reuse):
    ...
    with tf.compat.v1.variable_scope(block_name(1)):
      if simple_arch:
        ...
        x = tf.compat.v1.layers.dense(x, start_h*start_w*num_filters_fn(1),
        ...
        x = tf.pad(tensor=x, paddings=[[0] * 2, [start_h - 1] * 2, [start_w - 1] * 2, [0] * 2])
    ...
    for block_id in range(1, num_blocks + 1):
      with tf.compat.v1.variable_scope(block_name(block_id)):
        if simple_arch:
          lod = lods[block_id - 1]
          lod = tf.compat.v1.layers.conv2d(
              lod,
              colors,
              kernel_size=1,
              padding='SAME',
...
def discriminator(x,
  ...
  with tf.compat.v1.variable_scope(scope, reuse=reuse):
    ...
    with tf.compat.v1.variable_scope(block_name(1)):
      ...
      if simple_arch:
        x = tf.reshape(x, [tf.shape(input=x)[0], -1])  # flatten
        x = tf.compat.v1.layers.dense(x, num_filters_fn(0), name='last_conv',
                            kernel_initializer=he_init)
        x = tf.reshape(x, [tf.shape(input=x)[0], 1, 1, num_filters_fn(0)])
        x = tf.nn.relu(x)
      else:
      ...
      if simple_arch:
        logits = tf.compat.v1.layers.dense(x, 1, name='logits',
                                 kernel_initializer=he_init)
      else:
