...
class NSynthTFRecordDataset(BaseDataset):
  ...
  def provide_one_hot_labels(self, batch_size):
    ...
    indices = tf.reshape(
        tf.random.categorical(logits=tf.math.log([tf.cast(counts, dtype=tf.float32)]), num_samples=batch_size), [batch_size])
  ...
  def provide_dataset(self):
    ...
    def _parse_nsynth(record):
      ...
      example = tf.io.parse_single_example(serialized=record, features=features)
