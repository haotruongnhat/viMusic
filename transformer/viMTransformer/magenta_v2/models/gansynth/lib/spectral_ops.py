...
def unwrap(p, discont=np.pi, axis=-1):
  ...
  ddmod = tf.compat.v1.where(idx, tf.ones_like(ddmod) * np.pi, ddmod)
  ph_correct = ddmod - dd
  idx = tf.less(tf.abs(dd), discont)
  ddmod = tf.compat.v1.where(idx, tf.zeros_like(ddmod), dd)
...
def instantaneous_frequency(phase_angle, time_axis=-2, use_unwrap=True):
  ...
  if use_unwrap:
    ...
    dphase = tf.compat.v1.where(dphase > np.pi, dphase - 2 * np.pi, dphase)
    dphase = tf.compat.v1.where(dphase < -np.pi, dphase + 2 * np.pi, dphase)
...
def polar2rect(mag, phase_angle):
  ...
  mag = tf.complex(mag, tf.convert_to_tensor(value=0.0, dtype=mag.dtype))
  phase = tf.complex(tf.cos(phase_angle), tf.sin(phase_angle))
  return mag * phase


def random_phase_in_radians(shape, dtype):
  return np.pi * (2 * tf.random.uniform(shape, dtype=dtype) - 1.0)
...
def crop_or_pad(waves, length, channels):
  ...
  """
  waves = tf.convert_to_tensor(value=waves)
  batch_size = waves.shape[0].value
  waves_shape = tf.shape(input=waves)

  # Force audio length.
  pad = tf.maximum(0, length - waves_shape[1])
  right_pad = tf.cast(tf.cast(pad, dtype=tf.float32) / 2.0, dtype=tf.int32)
  left_pad = pad - right_pad
  waves = tf.pad(tensor=waves, paddings=[[0, 0], [left_pad, right_pad], [0, 0]])
  waves = waves[:, :length, :]

  # Force number of channels.
  num_repeats = tf.cast(
      tf.math.ceil(tf.cast(channels, dtype=tf.float32) / tf.cast(waves_shape[2], dtype=tf.float32)), dtype=tf.int32)
