...
def pixel_norm(images, epsilon=1.0e-8):
  ...
  return images * tf.math.rsqrt(
      tf.reduce_mean(input_tensor=tf.square(images), axis=3, keepdims=True) + epsilon)
...
def downscale_height(images, scale):
  ...
  if scale == 1:
    return images
  return tf.nn.avg_pool2d(
      input=images, ksize=[1, scale, 1, 1], strides=[1, scale, 1, 1], padding='VALID')
...
def minibatch_mean_stddev(x):
  ...
  del mean
  return tf.reduce_mean(input_tensor=tf.sqrt(var + 1e-6))
...
def debugprint(x, name=''):
  ...
  return tf.compat.v1.Print(x,
                  [tf.reduce_min(input_tensor=x), tf.reduce_mean(input_tensor=x), tf.reduce_max(input_tensor=x)],
                  name)
