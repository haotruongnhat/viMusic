...
def main(unused_argv):
  ...
  # Create summaries
  tf.compat.v1.summary.scalar('Train_Loss', m.vae_loss)
  tf.compat.v1.summary.scalar('Mean_Recon_LL', m.mean_recons)
  tf.compat.v1.summary.scalar('Mean_KL', m.mean_KL)
  scalar_summaries = tf.compat.v1.summary.merge_all()
  ...
  x_mean_summary = tf.compat.v1.summary.image(
      'Reconstruction', nn.tf_batch_image(x_mean_), max_outputs=1)
  x_summary = tf.compat.v1.summary.image('Original', nn.tf_batch_image(x_), max_outputs=1)
  sample_summary = tf.compat.v1.summary.image(
      'Sample', nn.tf_batch_image(x_mean_), max_outputs=1)
  # Summary writers
  train_writer = tf.compat.v1.summary.FileWriter(save_dir + '/vae_train', sess.graph)
  eval_writer = tf.compat.v1.summary.FileWriter(save_dir + '/vae_eval', sess.graph)
  ...
  for i in range(i_start, FLAGS.n_iters):
    ...
    tf.compat.v1.logging.info('Iter: %d, Loss: %d', i, res[1])
    ...
    if i % FLAGS.n_iters_per_save == 0:
      ...
      if smoothed_eval_loss < best_eval_loss:
        ...
        tf.compat.v1.logging.info('SAVING BEST! %s Iter: %d', save_name, i)
        m.vae_saver.save(sess, save_name)
        with tf.io.gfile.GFile(os.path.join(best_dir, 'best_ckpt_iters.txt'),
                           'w') as f:
          f.write('%d' % i)
