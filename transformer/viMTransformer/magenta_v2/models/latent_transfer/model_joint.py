...
class VAE(snt.AbstractModule):
  ...
  def _build(self, unused_input=None):
    ...
    x = tf.compat.v1.placeholder(tf.float32, shape=(None, n_latent))
    ...
    # ---------------------------------------------------------------------
    mu, sigma = encoder(x)
    mean_abs_mu, mean_abs_sigma = tf.reduce_mean(input_tensor=tf.abs(mu)), tf.reduce_mean(
        input_tensor=tf.abs(sigma))  # for summary only
    ...
    recons = tf.reduce_mean(input_tensor=tf.square(x_prime - x))
    mean_recons = tf.reduce_mean(input_tensor=recons)
    ...
    if beta == 0:
      ...
      if config['prior_loss'].lower() == 'KL'.lower():
        ...
        KL = tf.reduce_sum(input_tensor=KL_qp, axis=-1)
    ...
    if vae_vars:
      ...
      # Optimizer
      train_vae = tf.compat.v1.train.AdamOptimizer(learning_rate=vae_lr).minimize(
          vae_loss, var_list=vae_vars)

      # Savers
      vae_saver = tf.compat.v1.train.Saver(vae_vars, max_to_keep=100)
...
class Model(snt.AbstractModule):
  ...
  def _build(self, unused_input=None):
    ...
    z_hat = tf.compat.v1.placeholder(tf.float32, shape=(None, n_latent_shared))
    ...
    x_align_A = tf.compat.v1.placeholder(tf.float32, shape=(None, n_latent_A))
    x_align_B = tf.compat.v1.placeholder(tf.float32, shape=(None, n_latent_B))
    ...
    KL_align = tf.reduce_sum(input_tensor=KL_qp_align, axis=-1)
    mean_KL_align = tf.reduce_mean(input_tensor=KL_align)
    ...
    mean_recons_A_align = tf.reduce_mean(input_tensor=tf.square(x_prime_A_align - x_align_A))
    mean_recons_B_align = tf.reduce_mean(input_tensor=tf.square(x_prime_B_align - x_align_B))
    ...
    mean_recons_A_to_B_align = tf.reduce_mean(
        input_tensor=tf.square(x_A_to_B_align - x_align_B))
    mean_recons_B_to_A_align = tf.reduce_mean(
        input_tensor=tf.square(x_B_to_A_align - x_align_A))
    ...
    train_full = tf.compat.v1.train.AdamOptimizer(learning_rate=full_lr).minimize(
