...
def main(unused_argv):
  ...
  # Prepare summary
  train_writer = tf.compat.v1.summary.FileWriter(save_dir + '/transfer_train', sess.graph)
  scalar_summaries = tf.compat.v1.summary.merge([
  ...
  for i in tqdm(list(range(i_start, n_iters)), desc='training', unit=' batch'):
    ...
    if i % FLAGS.n_iters_per_save == 0:
      ...
      with tf.io.gfile.GFile(os.path.join(save_dir, 'ckpt_iters.txt'), 'w') as f:
        f.write('%d' % i)
    ...
    if i % FLAGS.n_iters_per_eval == 0:
      ...
      tf.io.gfile.makedirs(this_iter_sample_dir)
