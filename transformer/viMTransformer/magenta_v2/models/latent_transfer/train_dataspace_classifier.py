...
def main(unused_argv):
  ...
  accuracy = tf.reduce_mean(input_tensor=tf.cast(tf.equal(y_true, y_pred), tf.float32))

  tf.compat.v1.summary.scalar('Loss', m.classifier_loss)
  ...
  eval_writer = tf.compat.v1.summary.FileWriter(save_dir + '/eval', sess.graph)
  ...
  for i in range(i_start, FLAGS.n_iters):
    ...
    tf.compat.v1.logging.info('Iter: %d, Loss: %.2e', i, res[1])
