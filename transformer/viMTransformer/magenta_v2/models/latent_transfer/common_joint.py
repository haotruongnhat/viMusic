...
class ManualSummaryHelper(object):
  ...
  def _add_key_if_not_exists(self, key):
    ...
    if key in self._key_to_ph_summary_tuple:
      return
    placeholder = tf.compat.v1.placeholder(tf.float32, shape=(), name=key + '_ph')
    summary = tf.compat.v1.summary.scalar(key, placeholder)
...
def load_dataset(config_name, exp_uid):
  ...
  tf.compat.v1.logging.info('index_grouped_by_label size: %s',
                  [len(_) for _ in index_grouped_by_label])

  tf.compat.v1.logging.info('train loaded from %s', path_train)
  tf.compat.v1.logging.info('train shapes: mu = %s, sigma = %s', train_mu.shape,
                  train_sigma.shape)
...
class ModelWaveGANHelper(object):
  ...
  def build(self):
    ...
    with graph_sc09_gan.as_default():
      ...
      sess_sc09_gan = tf.compat.v1.Session(graph=graph_sc09_gan)
      saver_gan = tf.compat.v1.train.import_meta_graph(
    ...
    with graph_sc09_class.as_default():
      ...
      sess_sc09_class = tf.compat.v1.Session(graph=graph_sc09_class)
      saver_class = tf.compat.v1.train.import_meta_graph(
