...
class Model(snt.AbstractModule):
  ...
  def _build(self, unused_input=None):
    ...
    if dataset_is_mnist_family(config['dataset']):
      ...
      attr_loss_fn = tf.compat.v1.losses.softmax_cross_entropy
      ...
      attr_loss_fn = tf.compat.v1.losses.sigmoid_cross_entropy
    ...
    # Attributes
    labels = tf.compat.v1.placeholder(tf.int32, shape=(None, n_labels), name='labels')
    # Real / fake label reward
    r = tf.compat.v1.placeholder(tf.float32, shape=(None, 1), name='D_label')
    # Transform through optimization
    z0 = tf.compat.v1.placeholder(tf.float32, shape=(None, n_latent), name='z0')
    ...
    if config['dataset'] == 'CELEBA':
      recons = tf.reduce_sum(input_tensor=p_x.log_prob(x), axis=[1, 2, 3])
    else:
      recons = tf.reduce_sum(input_tensor=p_x.log_prob(x), axis=[-1])
    ...
    train_classifier = tf.compat.v1.train.AdamOptimizer(
    ...
    classifier_saver = tf.compat.v1.train.Saver(classifier_vars, max_to_keep=1000)
