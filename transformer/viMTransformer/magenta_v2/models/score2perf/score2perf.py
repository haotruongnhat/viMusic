...
class Score2PerfProblem(problem.Problem):
  ...
  def example_reading_spec(self):
    ...
    for name, _ in self.score_encoders():
      data_fields[name] = tf.io.VarLenFeature(tf.int64)
  ...
  def preprocess_example(self, example, mode, hparams):
    ...
    if self.random_crop_in_train and mode == tf.estimator.ModeKeys.TRAIN:
      ...
      max_offset = tf.maximum(
          tf.shape(input=example['targets'])[0] - hparams.max_target_seq_length, 0)
      offset = tf.cond(
          pred=max_offset > 0,
      ...
      assert not self.has_inputs
      length = tf.shape(input=example['targets'])[0]
      extra_length = tf.math.mod(length, hparams.max_target_seq_length)
      examples = {
          'targets': tf.reshape(
