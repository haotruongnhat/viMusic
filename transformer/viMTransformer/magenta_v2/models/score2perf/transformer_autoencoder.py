...
def perf_transformer_encode(encoder_function, inputs, target_space, hparams,
  ...
  encoder_input = tf.nn.dropout(encoder_input,
                                1 - (1.0 - hparams.layer_prepostprocess_dropout))
  ...
  if not baseline:
    encoder_output = tf.math.reduce_mean(
        input_tensor=encoder_output, axis=1, keepdims=True)
    encoder_decoder_attention_bias = tf.math.reduce_mean(
        input_tensor=encoder_decoder_attention_bias, axis=-1, keepdims=True)
...
def mel_perf_transformer_encode(encoder_function, perf_inputs, mel_inputs,
  ...
  perf_mean_vector = tf.math.reduce_mean(
      input_tensor=perf_encoder_output, axis=1, keepdims=True)
  ...
  if hparams.aggregation == "sum":
    ...
    perf_mean_bias = tf.math.reduce_mean(input_tensor=perf_encdec_attention_bias,
    ...
    dynamic_val = tf.shape(input=mel_encoder_output)[1]
    shp = tf.convert_to_tensor(value=[1, dynamic_val, 1], dtype=tf.int32)
...
class MelodyPerformanceTransformer(Transformer):
  ...
  def _slow_greedy_infer(self, features, decode_length):
    ...
    def infer_step(recent_output, recent_logits, unused_loss):
      ...
      padded = tf.pad(tensor=recent_output, paddings=[[0, 0], [0, 1], [0, 0], [0, 0]])
    ...
    def while_exit_cond(result, logits, loss):  # pylint: disable=unused-argument
      ...
      if self._problem_hparams.stop_at_eos:
        ...
        not_eos = tf.cond(
            ...
            pred=tf.not_equal(length, 0),
            true_fn=fn_not_eos,
            false_fn=lambda: True,
        )

        return tf.cond(
            pred=tf.equal(batch_size, 1),
            # If batch_size == 1, we check EOS for early stopping.
            true_fn=lambda: tf.logical_and(not_overflow, not_eos),
            # Else, just wait for max length
            false_fn=lambda: not_overflow)
      return not_overflow
