...
class RnnNade(object):
  ...
  def _get_state(self,
    ...
    if lengths is None:
      lengths = tf.tile(tf.shape(input=inputs)[1:2], [batch_size])
  ...
  def log_prob(self, sequences, lengths=None):
    ...
    inputs = tf.pad(tensor=inputs, paddings=[[0, 0], [1, 0], [0, 0]])
  ...
  def zero_state(self, batch_size):
    ...
    """
    with tf.compat.v1.name_scope('RnnNadeZeroState', values=[batch_size]):
...
def get_build_graph_fn(mode, config, sequence_example_file_paths=None):
  ...
  def build():
    ...
    if mode in ('train', 'eval'):
      ...
      inputs_flat = tf.cast(
          magenta.common.flatten_maybe_padded_sequences(inputs, lengths), dtype=tf.float32)
      predictions_flat = tf.cast(tf.greater_equal(cond_probs, .5), dtype=tf.float32)

      if mode == 'train':
        loss = tf.reduce_mean(input_tensor=-log_probs)
        perplexity = tf.reduce_mean(input_tensor=tf.exp(log_probs))
        correct_predictions = tf.cast(
            tf.equal(inputs_flat, predictions_flat), dtype=tf.float32)
        accuracy = tf.reduce_mean(input_tensor=correct_predictions)
        precision = (tf.reduce_sum(input_tensor=inputs_flat * predictions_flat) /
                     tf.reduce_sum(input_tensor=predictions_flat))
        recall = (tf.reduce_sum(input_tensor=inputs_flat * predictions_flat) /
                  tf.reduce_sum(input_tensor=inputs_flat))
        ...
        vars_to_summarize, update_ops = contrib_metrics.aggregate_metric_map({
            'loss':
                tf.compat.v1.metrics.mean(-log_probs),
            'metrics/perplexity':
                tf.compat.v1.metrics.mean(tf.exp(log_probs)),
            'metrics/accuracy':
                tf.compat.v1.metrics.accuracy(inputs_flat, predictions_flat),
            'metrics/precision':
                tf.compat.v1.metrics.precision(inputs_flat, predictions_flat),
            'metrics/recall':
                tf.compat.v1.metrics.recall(inputs_flat, predictions_flat),
        })
      ...
      tf.compat.v1.add_to_collection('sample', samples)
      tf.compat.v1.add_to_collection('log_prob', log_prob)
