...
def cudnn_lstm_layer(inputs,
  ...
  inputs_t = tf.transpose(a=inputs, perm=[1, 0, 2])
  if lengths is not None:
    all_outputs = [inputs_t]
    for i in range(stack_size):
      with tf.compat.v1.variable_scope('stack_' + str(i)):
        ...
        if bidirectional:
          ...
          inputs_reversed = tf.reverse_sequence(
              input=all_outputs[-1], seq_lengths=lengths, seq_axis=0, batch_axis=1)
          outputs_bw, _ = lstm_bw(
              inputs_reversed, (h_bw, c_bw), training=is_training)

          outputs_bw = tf.reverse_sequence(
              input=outputs_bw, seq_lengths=lengths, seq_axis=0, batch_axis=1)
    ...
    # more resnet like
    return tf.transpose(a=all_outputs[-1], perm=[1, 0, 2])
  else:
...
def model_fn(features, labels, mode, params, config):
  ...
  with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=is_training):
    with tf.compat.v1.variable_scope('onsets'):
      ...
      if is_training:
        ...
        tf.compat.v1.losses.add_loss(tf.reduce_mean(input_tensor=onset_losses))
        losses['onset'] = onset_losses
    with tf.compat.v1.variable_scope('offsets'):
      ...
      if is_training:
        ...
        tf.compat.v1.losses.add_loss(tf.reduce_mean(input_tensor=offset_losses))
        losses['offset'] = offset_losses
    with tf.compat.v1.variable_scope('velocity'):
      ...
      if is_training:
        ...
        velocity_loss = tf.reduce_sum(
            input_tensor=onset_labels_flat *
            tf.square(velocity_labels_flat - velocity_values_flat),
            axis=1)
        tf.compat.v1.losses.add_loss(tf.reduce_mean(input_tensor=velocity_loss))
        losses['velocity'] = velocity_loss

    with tf.compat.v1.variable_scope('frame'):
    ...
    if is_training:
      ...
      tf.compat.v1.losses.add_loss(tf.reduce_mean(input_tensor=frame_losses))
      ...
      if hparams.activation_loss:
        ...
        tf.compat.v1.losses.add_loss(tf.reduce_mean(input_tensor=activation_losses))
  ...
  metric_ops = {k: tf.compat.v1.metrics.mean(v) for k, v in metrics_values.items()}
