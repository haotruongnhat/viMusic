...
def calculate_frame_metrics(frame_labels, frame_predictions):
  ...
  frame_true_positives = tf.reduce_sum(input_tensor=tf.cast(tf.logical_and(
      tf.equal(frame_labels_bool, True),
      tf.equal(frame_predictions_bool, True)), dtype=tf.float32))
  frame_false_positives = tf.reduce_sum(input_tensor=tf.cast(tf.logical_and(
      tf.equal(frame_labels_bool, False),
      tf.equal(frame_predictions_bool, True)), dtype=tf.float32))
  frame_false_negatives = tf.reduce_sum(input_tensor=tf.cast(tf.logical_and(
      tf.equal(frame_labels_bool, True),
      tf.equal(frame_predictions_bool, False)), dtype=tf.float32))
  frame_accuracy = (
      tf.reduce_sum(
          input_tensor=tf.cast(tf.equal(frame_labels_bool, frame_predictions_bool), dtype=tf.float32)) /
      tf.cast(tf.size(input=frame_labels), tf.float32))

  frame_precision = tf.compat.v1.where(
      tf.greater(frame_true_positives + frame_false_positives, 0),
      tf.compat.v1.div(frame_true_positives,
             frame_true_positives + frame_false_positives),
      0)
  frame_recall = tf.compat.v1.where(
...
def calculate_metrics(frame_predictions,
  ...
  (note_precision, note_recall, note_f1, note_with_velocity_precision,
   ...
   note_with_offsets_precision, note_with_offsets_recall, note_with_offsets_f1,
   note_with_offsets_velocity_precision, note_with_offsets_velocity_recall,
   note_with_offsets_velocity_f1, processed_frame_predictions) = tf.compat.v1.py_func(
       functools.partial(
  ...
  if pitch_map:
    for pitch, name in pitch_map.items():
      (note_precision, note_recall, note_f1, note_with_velocity_precision,
       ...
       note_with_offsets_velocity_recall, note_with_offsets_velocity_f1,
       processed_frame_predictions) = tf.compat.v1.py_func(
           functools.partial(
               _calculate_metrics_py,
               hparams=hparams,
               min_pitch=min_pitch,
