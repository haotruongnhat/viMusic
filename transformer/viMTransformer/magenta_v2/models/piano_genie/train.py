...
def main(unused_argv):
  ...
  if cfg.stp_emb_vq:
    tf.compat.v1.summary.scalar("codebook_perplexity",
    ...
    tf.compat.v1.summary.scalar("loss_vqvae", model_dict["stp_emb_vq_loss"])

  # Summarize integer-quantized step embeddings
  if cfg.stp_emb_iq:
    tf.compat.v1.summary.scalar("discrete_perplexity",
                      model_dict["stp_emb_iq_discrete_ppl"])
    tf.compat.v1.summary.scalar("iq_valid_p", model_dict["stp_emb_iq_valid_p"])
    tf.compat.v1.summary.image(
        "genie",
        util.discrete_to_piano_roll(
    ...
    tf.compat.v1.summary.scalar("loss_iq_range", model_dict["stp_emb_iq_range_penalty"])
    tf.compat.v1.summary.scalar("loss_iq_contour",
                      model_dict["stp_emb_iq_contour_penalty"])
    tf.compat.v1.summary.scalar("loss_iq_deviate",
                      model_dict["stp_emb_iq_deviate_penalty"])

  if cfg.stp_emb_vq or cfg.stp_emb_iq:
    tf.compat.v1.summary.scalar("contour_violation", model_dict["contour_violation"])
    tf.compat.v1.summary.scalar("deviate_violation", model_dict["deviate_violation"])

  # Summarize VAE sequence embeddings
  if cfg.seq_emb_vae:
    tf.compat.v1.summary.scalar("loss_kl", model_dict["seq_emb_vae_kl"])
  ...
  if cfg.dec_pred_velocity:
    tf.compat.v1.summary.scalar("loss_recons_velocity",
                      model_dict["dec_recons_velocity_loss"])
    tf.compat.v1.summary.scalar("ppl_recons_velocity",
                      tf.exp(model_dict["dec_recons_velocity_loss"]))

  # Reconstruction loss
  tf.compat.v1.summary.scalar("loss_recons", model_dict["dec_recons_loss"])
  tf.compat.v1.summary.scalar("ppl_recons", tf.exp(model_dict["dec_recons_loss"]))
  ...
  # Construct optimizer
  opt = tf.compat.v1.train.AdamOptimizer(learning_rate=cfg.train_lr)
