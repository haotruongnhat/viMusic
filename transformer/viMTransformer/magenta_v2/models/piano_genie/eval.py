...
def main(unused_argv):
  if not tf.io.gfile.isdir(FLAGS.eval_dir):
  ...
  # Build model
  with tf.compat.v1.variable_scope("phero_model"):
    model_dict = build_genie_model(
        ...
        cfg.eval_seq_len,
        is_training=False)
  genie_vars = tf.compat.v1.get_collection(
      tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, scope="phero_model")

  # Build gold model
  eval_gold = False
  if cfg.stp_emb_vq or cfg.stp_emb_iq:
    eval_gold = True
    with tf.compat.v1.variable_scope("phero_model", reuse=True):
      gold_feat_dict = {
          "midi_pitches": tf.compat.v1.placeholder(tf.int32, [1, None]),
          "velocities": tf.compat.v1.placeholder(tf.int32, [1, None]),
          "delta_times_int": tf.compat.v1.placeholder(tf.int32, [1, None])
      }
      gold_seq_maxlen = gold.gold_longest()
      gold_seq_varlens = tf.compat.v1.placeholder(tf.int32, [1])
      gold_buttons = tf.compat.v1.placeholder(tf.int32, [1, None])
    ...
    weighted_avg = lambda t, m: tf.reduce_sum(input_tensor=t * m) / tf.reduce_sum(input_tensor=m)

    gold_diff_l2 = weighted_avg(gold_diff_l2, gold_mask)
    gold_diff_l1 = weighted_avg(gold_diff_l1, gold_mask)

    gold_diff_l2_placeholder = tf.compat.v1.placeholder(tf.float32, [None])
    gold_diff_l1_placeholder = tf.compat.v1.placeholder(tf.float32, [None])
  ...
  for name in summary_name_to_batch_tensor:
    placeholder = tf.compat.v1.placeholder(tf.float32, [None])
    summary_name_to_placeholder[name] = placeholder
    summaries.append(tf.compat.v1.summary.scalar(name, tf.reduce_mean(input_tensor=placeholder)))
  if eval_gold:
    summary_name_to_placeholder["gold_diff_l2"] = gold_diff_l2_placeholder
    summaries.append(
        tf.compat.v1.summary.scalar("gold_diff_l2",
                          tf.reduce_mean(input_tensor=gold_diff_l2_placeholder)))
    summary_name_to_placeholder["gold_diff_l1"] = gold_diff_l1_placeholder
    summaries.append(
        tf.compat.v1.summary.scalar("gold_diff_l1",
                          tf.reduce_mean(input_tensor=gold_diff_l1_placeholder)))
  ...
  saver = tf.compat.v1.train.Saver(genie_vars + [step], max_to_keep=None)
