...
def demidify(pitches):
  ...
  assertions = [
      tf.compat.v1.assert_greater_equal(pitches, 21),
      tf.compat.v1.assert_less_equal(pitches, 108)
  ]
...
def remidify(pitches):
  ...
  assertions = [
      tf.compat.v1.assert_greater_equal(pitches, 0),
      tf.compat.v1.assert_less_equal(pitches, 87)
  ]
  with tf.control_dependencies(assertions):
    return pitches + 21
...
def discrete_to_piano_roll(categorical, dim, dilation=1, colorize=True):
  ...
  if dilation > 1:
    old_height = tf.shape(input=piano_roll)[1]
    old_width = tf.shape(input=piano_roll)[2]

    piano_roll = tf.image.resize(
        piano_roll, [old_height * dilation, old_width], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
