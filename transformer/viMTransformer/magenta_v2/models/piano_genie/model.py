...
def simple_lstm_encoder(features,
  ...
  with tf.compat.v1.variable_scope("rnn_input"):
    x = tf.compat.v1.layers.dense(x, rnn_nunits)
  ...
  with tf.compat.v1.variable_scope("rnn"):
    if rnn_bidirectional:
      (x_fw, x_bw), (state_fw, state_bw) = tf.compat.v1.nn.bidirectional_dynamic_rnn(
          cell_fw=cell,
          cell_bw=cell,
          inputs=x,
      ...
      x, state = tf.compat.v1.nn.dynamic_rnn(
...
def simple_lstm_decoder(features,
  ...
  with tf.compat.v1.variable_scope("rnn"):
    initial_state = cell.zero_state(batch_size, dtype)
    x, final_state = tf.compat.v1.nn.dynamic_rnn(
        cell=cell,
        inputs=x,
        sequence_length=seq_lens,
...
def weighted_avg(t, mask=None, axis=None, expand_mask=False):
  if mask is None:
    return tf.reduce_mean(input_tensor=t, axis=axis)
  else:
    if expand_mask:
      mask = tf.expand_dims(mask, axis=-1)
    return tf.reduce_sum(
        input_tensor=tf.multiply(t, mask), axis=axis) / tf.reduce_sum(
            input_tensor=mask, axis=axis)
...
def build_genie_model(feat_dict,
  ...
  if cfg.stp_emb_unconstrained:
    with tf.compat.v1.variable_scope("stp_emb_unconstrained"):
      stp_emb_unconstrained = tf.compat.v1.layers.dense(
  ...
  if cfg.stp_emb_vq:
    ...
    with tf.compat.v1.variable_scope("stp_emb_vq"):
      with tf.compat.v1.variable_scope("pre_vq"):
        # pre_vq_encoding is tf.float32 of [batch_size, seq_len, embedding_dim]
        pre_vq_encoding = tf.compat.v1.layers.dense(enc_stp, cfg.stp_emb_vq_embedding_dim)

      with tf.compat.v1.variable_scope("quantizer"):
        ...
        stp_emb_vq_discrete = tf.reshape(
            tf.argmax(input=vq_vae_output["encodings"], axis=1, output_type=tf.int32),
            [batch_size, seq_len])
        stp_emb_vq_codebook = tf.transpose(a=vq_vae.embeddings)
    ...
    out_dict["stp_emb_vq_quantized_lookup"] = tf.nn.embedding_lookup(
        params=stp_emb_vq_codebook, ids=stp_emb_vq_discrete)

  # Integer-quantized step embeddings with straight-through
  if cfg.stp_emb_iq:
    with tf.compat.v1.variable_scope("stp_emb_iq"):
      with tf.compat.v1.variable_scope("pre_iq"):
        # pre_iq_encoding is tf.float32 of [batch_size, seq_len]
        pre_iq_encoding = tf.compat.v1.layers.dense(enc_stp, 1)[:, :, 0]
      ...
      with tf.compat.v1.variable_scope("quantizer"):
        ...
        stp_emb_iq_discrete_ppl = tf.exp(-tf.reduce_sum(
            input_tensor=stp_emb_iq_avg_probs * tf.math.log(stp_emb_iq_avg_probs + 1e-10)))
  ...
  if cfg.seq_emb_unconstrained:
    with tf.compat.v1.variable_scope("seq_emb_unconstrained"):
      seq_emb_unconstrained = tf.compat.v1.layers.dense(
  ...
  if cfg.seq_emb_vae:
    with tf.compat.v1.variable_scope("seq_emb_vae"):
      seq_emb_vae = tf.compat.v1.layers.dense(enc_seq, cfg.seq_emb_vae_embedding_dim * 2)

      mean = seq_emb_vae[:, :cfg.seq_emb_vae_embedding_dim]
      stddev = 1e-6 + tf.nn.softplus(
          seq_emb_vae[:, cfg.seq_emb_vae_embedding_dim:])
      seq_emb_vae = mean + stddev * tf.random.normal(
          tf.shape(input=mean), 0, 1, dtype=dtype)

      kl = tf.reduce_mean(input_tensor=0.5 * tf.reduce_sum(
          input_tensor=tf.square(mean) + tf.square(stddev) - tf.math.log(1e-8 + tf.square(stddev))
          - 1,
          axis=1))
  ...
  if cfg.lor_emb_unconstrained:
    ...
    with tf.compat.v1.variable_scope("lor_emb_unconstrained"):
      ...
      lor_emb_unconstrained = tf.compat.v1.layers.dense(
  ...
  if cfg.dec_autoregressive:
    ...
    last_pitches = tf.pad(
        tensor=last_pitches, paddings=[[0, 0], [1, 0]],
    ...
    if cfg.dec_pred_velocity:
      ...
      last_velocities = tf.pad(tensor=last_velocities, paddings=[[0, 0], [1, 0]])
      dec_feats.append(
  ...
  with tf.compat.v1.variable_scope("decoder"):
    ...
    with tf.compat.v1.variable_scope("pitches"):
      dec_recons_logits = tf.compat.v1.layers.dense(dec_stp, 88)
    ...
    out_dict["dec_recons_preds"] = tf.argmax(
        input=dec_recons_logits, output_type=tf.int32, axis=-1)
    out_dict["dec_recons_midi_preds"] = util.remidify(
        out_dict["dec_recons_preds"])
    out_dict["dec_recons_loss"] = dec_recons_loss

    if cfg.dec_pred_velocity:
      with tf.compat.v1.variable_scope("velocities"):
        dec_recons_velocity_logits = tf.compat.v1.layers.dense(
  ...
  if cfg.stp_emb_vq or cfg.stp_emb_iq:
    ...
    contour_violation = tf.reduce_mean(input_tensor=tf.cast(tf.less(dx * dy, 0), tf.float32))
