...
def _checkpoint_file_exists(checkpoint_file_or_prefix):
  ...
  return (tf.io.gfile.exists(checkpoint_file_or_prefix) or
          tf.io.gfile.exists(checkpoint_file_or_prefix + '.index'))
...
class BaseSequenceGenerator(object):
  ...
  def initialize(self):
    ...
    if self._checkpoint is not None:
      ...
      if tf.io.gfile.isdir(checkpoint_file):
      ...
      if (not _checkpoint_file_exists(self._checkpoint) or
          tf.io.gfile.isdir(checkpoint_file)):
        raise SequenceGeneratorError(
      ...
      try:
        ...
        with tf.io.gfile.GFile(checkpoint_filename, 'wb') as f:
  ...
  def create_bundle_file(self, bundle_file, bundle_description=None):
    ...
    try:
      ...
      with tf.io.gfile.GFile(metagraph_filename, 'rb') as f:
        bundle.metagraph_file = f.read()

      with tf.io.gfile.GFile(bundle_file, 'wb') as f:
