...
class LstmEncoder(base_model.BaseEncoder):
  ...
  def encode(self, sequence, sequence_length):
    ...
    sequence = tf.transpose(a=sequence, perm=[1, 0, 2])
    if self._use_cudnn:
...
class BidirectionalLstmEncoder(base_model.BaseEncoder):
  ...
  def build(self, hparams, is_training=True, name_or_scope='encoder'):
    ...
    if isinstance(name_or_scope, tf.compat.v1.VariableScope):
    ...
    for i, layer_size in enumerate(hparams.enc_rnn_size):
      if self._use_cudnn:
        cells_fw.append(lstm_utils.cudnn_lstm_layer(
            ...
            name_or_scope=tf.compat.v1.VariableScope(
                reuse,
...
class HierarchicalLstmEncoder(base_model.BaseEncoder):
  ...
  def build(self, hparams, is_training=True):
    ...
    for i, l in enumerate(self._level_lengths):
      num_splits //= l
      tf.compat.v1.logging.info('Level %d splits: %d', i, num_splits)
      h_encoder = self._core_encoder_cls()
      h_encoder.build(
          hparams, is_training,
          name_or_scope=tf.compat.v1.VariableScope(
              tf.compat.v1.AUTO_REUSE, 'encoder/hierarchical_level_%d' % i))
  ...
  def encode(self, sequence, sequence_length):
    ...
    with tf.control_dependencies([tf.compat.v1.assert_equal(tf.shape(input=sequence)[1], 1)]):
      return sequence[:, 0]
...
class BaseLstmDecoder(base_model.BaseDecoder):
  ...
  def _decode(self, z, helper, input_shape, max_length=None):
    ...
    if  self._cudnn_dec_lstm and type(helper) is seq2seq.TrainingHelper:  # pylint:disable=unidiomatic-typecheck
      rnn_output, _ = self._cudnn_dec_lstm(
          tf.transpose(a=helper.inputs, perm=[1, 0, 2]),
      ...
      results = lstm_utils.LstmDecodeResults(
          ...
          rnn_output=tf.transpose(a=rnn_output, perm=[1, 0, 2]),
  ...
  def reconstruction_loss(self, x_input, x_target, x_length, z=None,
    ...
    repeated_z = tf.tile(
        tf.expand_dims(z, axis=1), [1, tf.shape(input=x_input)[1], 1])

    has_control = c_input is not None
    if c_input is None:
      c_input = tf.zeros([batch_size, tf.shape(input=x_input)[1], 0])
    ...
    if sampling_probability_static == 0.0:
      ...
      if has_z or has_control:
        auxiliary_inputs = tf.zeros([batch_size, tf.shape(input=x_input)[1], 0])
        if has_z:
    ...
    for i in range(batch_size):
      ...
      r_losses.append(tf.reduce_sum(input_tensor=r_loss[b:e]))
...
class CategoricalLstmDecoder(BaseLstmDecoder):
  ...
  def _flat_reconstruction_loss(self, flat_x_target, flat_rnn_output):
    ...
    flat_truth = tf.argmax(input=flat_x_target, axis=1)
    flat_predictions = tf.argmax(input=flat_logits, axis=1)
    r_loss = tf.nn.softmax_cross_entropy_with_logits(
        labels=tf.stop_gradient(flat_x_target), logits=flat_logits)

    metric_map = {
        'metrics/accuracy':
            tf.compat.v1.metrics.accuracy(flat_truth, flat_predictions),
  ...
  def sample(self, n, max_length=None, z=None, c_input=None, temperature=None,
    ...
    if beam_width is None:
      if end_token is None:
        ...
        end_fn = lambda x: tf.equal(tf.argmax(input=x, axis=-1), end_token)
    ...
    if temperature is not None:
      tf.compat.v1.logging.warning('`temperature` is ignored when using beam search.')
    # Use a dummy Z in unconditional case.
    z = tf.zeros((n, 0), tf.float32) if z is None else z
    ...
    if start_inputs is None:
      ...
      start_tokens = tf.argmax(input=start_inputs, axis=-1, output_type=tf.int32)
    ...
    def embedding_fn(tokens):
      ...
      next_inputs = tf.cond(
          pred=tf.less(tokens[0, 0], 0),
          true_fn=lambda: tf.zeros([n, beam_width, self._output_depth]),
          false_fn=lambda: tf.one_hot(tokens, self._output_depth))
...
class MultiOutCategoricalLstmDecoder(CategoricalLstmDecoder):
  ...
  def _flat_reconstruction_loss(self, flat_x_target, flat_rnn_output):
    ...
    return tf.reduce_sum(input_tensor=losses, axis=0), metric_map
...
class SplitMultiOutLstmDecoder(base_model.BaseDecoder):
  ...
  def build(self, hparams, output_depth, is_training=True):
    ...
    for i, (cd, od) in enumerate(zip(self._core_decoders, self._output_depths)):
      with tf.compat.v1.variable_scope('core_decoder_%d' % i):
  ...
  def reconstruction_loss(self, x_input, x_target, x_length, z=None,
    ...
    for metric_name in metric_maps[0]:
      ...
      merged_metric_map[metric_name] = (
          tf.reduce_mean(input_tensor=metric_values), tf.no_op())

    return (tf.reduce_sum(input_tensor=r_losses, axis=0),
            merged_metric_map,
    ...
    metric_map = {
        'metrics/accuracy':
            tf.compat.v1.metrics.mean(
                tf.reduce_all(input_tensor=tf.equal(flat_truth, flat_predictions), axis=-1)),
        'metrics/recall':
            tf.compat.v1.metrics.recall(flat_truth, flat_predictions),
        'metrics/precision':
            tf.compat.v1.metrics.precision(flat_truth, flat_predictions),
    }
...
class HierarchicalLstmDecoder(base_model.BaseDecoder):
  ...
  def build(self, hparams, output_depth, is_training=True):
    ...
    with tf.compat.v1.variable_scope('core_decoder', reuse=tf.compat.v1.AUTO_REUSE):
  ...
  def _hierarchical_decode(self, z, base_decode_fn):
    ...
    def recursive_decode(initial_input, path=None):
      ...
      scope = tf.compat.v1.VariableScope(
          tf.compat.v1.AUTO_REUSE, 'decoder/hierarchical_level_%d' % level)
  ...
  def _reshape_to_hierarchy(self, t):
    ...
    return tf.transpose(a=hier_t, perm=perm)
  ...
  def reconstruction_loss(self, x_input, x_target, x_length, z=None,
    ...
    def base_train_fn(embedding, hier_index):
      ...
      if self._hierarchical_encoder:
        ...
        samples = tf.pad(
            tensor=samples,
            paddings=[(0, 0), (0, split_size - tf.shape(input=samples)[1]), (0, 0)])
  ...
  def sample(self, n, max_length=None, z=None, c_input=None,
    ...
    all_samples = tf.concat(
        [tf.pad(tensor=s, paddings=[(0, 0), (0, core_max_length - tf.shape(input=s)[1]), (0, 0)])
...
class GrooveLstmDecoder(BaseLstmDecoder):
  ...
  def _flat_reconstruction_loss(self, flat_x_target, flat_rnn_output):
    ...
    hits_loss = tf.reduce_sum(input_tensor=tf.compat.v1.losses.log_loss(
        labels=target_hits, predictions=output_hits,
        reduction=tf.compat.v1.losses.Reduction.NONE), axis=1)

    velocities_loss = tf.reduce_sum(input_tensor=tf.compat.v1.losses.mean_squared_error(
        target_velocities, output_velocities,
        reduction=tf.compat.v1.losses.Reduction.NONE), axis=1)

    offsets_loss = tf.reduce_sum(input_tensor=tf.compat.v1.losses.mean_squared_error(
    ...
    metric_map = {
        'metrics/hits_loss':
            tf.compat.v1.metrics.mean(hits_loss),
        'metrics/velocities_loss':
            tf.compat.v1.metrics.mean(velocities_loss),
        'metrics/offsets_loss':
            tf.compat.v1.metrics.mean(offsets_loss)
    }
