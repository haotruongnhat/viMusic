  ...
  def _to_tensors(self, note_sequence):
    ...
    for note in quantized_sequence.notes:
      ...
      if i > MAX_INSTRUMENT_NUMBER:
        tf.compat.v1.logging.warning('Skipping invalid instrument number: %d', i)
        continue
      if note.is_drum:
    ...
    for i in (instruments_by_type[self.InstrumentType.MEL] +
      ...
      if tensors.outputs:
        ...
        if encoded_chords is None:
          ...
          tf.compat.v1.logging.warning('Trio chords disagreement between instruments.')
      else:
        coverage[:, i] = False
...
def count_examples(examples_path, tfds_name, data_converter,
  ...
  def _file_generator():
    filenames = tf.io.gfile.glob(examples_path)
    for f in filenames:
      tf.compat.v1.logging.info('Counting examples in %s.', f)
  ...
  tf.compat.v1.logging.info('Total examples: %d', num_examples)
  return num_examples
...
def get_dataset(
  ...
  if examples_path:
    tf.compat.v1.logging.info('Reading examples from file: %s', examples_path)
    num_files = len(tf.io.gfile.glob(examples_path))
    if not num_files:
      raise ValueError(
    ...
    tf.compat.v1.logging.info('Reading examples from TFDS: %s', config.tfds_name)
    dataset = tfds.load(
        config.tfds_name,
