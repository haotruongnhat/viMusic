...
class MusicVAE(object):
  ...
  def build(self, hparams, output_depth, is_training):
    ...
    """
    tf.compat.v1.logging.info('Building MusicVAE model with %s, %s, and hparams:\n%s',
  ...
  def encode(self, sequence, sequence_length, control_sequence=None):
    ...
    if control_sequence is not None:
      control_sequence = tf.cast(control_sequence, dtype=tf.float32)
  ...
  def _compute_model_loss(
    ...
    input_sequence = tf.cast(input_sequence, dtype=tf.float32)
    output_sequence = tf.cast(output_sequence, dtype=tf.float32)

    max_seq_len = tf.minimum(tf.shape(input=output_sequence)[1], hparams.max_seq_len)
    ...
    x_input = tf.pad(tensor=output_sequence[:, :max_seq_len - 1],
    ...
    beta = ((1.0 - tf.pow(hparams.beta_rate, tf.cast(self.global_step, dtype=tf.float32)))
            * hparams.max_beta)
    self.loss = tf.reduce_mean(input_tensor=r_loss) + beta * tf.reduce_mean(input_tensor=kl_cost)
  ...
  def train(self, input_sequence, output_sequence, sequence_length,
    ...
    lr = ((hparams.learning_rate - hparams.min_learning_rate) *
          tf.pow(hparams.decay_rate, tf.cast(self.global_step, dtype=tf.float32)) +
    ...
    for n, t in scalars_to_summarize.items():
      tf.compat.v1.summary.scalar(n, tf.reduce_mean(input_tensor=t))
  ...
  def eval(self, input_sequence, output_sequence, sequence_length,
    ...
    for n, t in scalars_to_summarize.iteritems():
      metric_map[n] = tf.compat.v1.metrics.mean(t)
