...
def set_final(sequence, sequence_length, values, time_major=False):
  ...
  sequence_batch_major = (
      sequence if not time_major else tf.transpose(a=sequence, perm=[1, 0, 2]))
  ...
  sequence_batch_major = (
      ...
      tf.scatter_nd(final_index, values, tf.shape(input=sequence_batch_major)))
  if time_major:
    return tf.transpose(a=sequence_batch_major, perm=[1, 0, 2])
...
def get_sampling_probability(hparams, is_training):
  ...
  tf.compat.v1.summary.scalar('sampling_probability', sampling_probability)
...
def maybe_split_sequence_lengths(sequence_length, num_splits, total_length):
  ...
  if sequence_length.shape.ndims == 1:
    ...
    with tf.control_dependencies(
        [tf.Assert(
            tf.reduce_all(
                input_tensor=tf.logical_or(tf.equal(sequence_length, 0),
