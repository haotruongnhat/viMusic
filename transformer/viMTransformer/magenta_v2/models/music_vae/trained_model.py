...
class TrainedModel(object):
  ...
  def __init__(self, config, batch_size, checkpoint_dir_or_path=None,
    ...
    if tf.io.gfile.isdir(checkpoint_dir_or_path):
    ...
    with tf.Graph().as_default():
      ...
      model.build(
          ...
          is_training=False)
      # Input placeholders
      self._temperature = tf.compat.v1.placeholder(tf.float32, shape=())

      if self._config.hparams.z_size:
        self._z_input = tf.compat.v1.placeholder(
            tf.float32, shape=[batch_size, self._config.hparams.z_size])
      else:
        self._z_input = None

      if self._config.data_converter.control_depth > 0:
        self._c_input = tf.compat.v1.placeholder(
      ...
      self._controls = tf.compat.v1.placeholder(
          tf.float32,
          shape=[batch_size, None, self._config.data_converter.control_depth])
      self._inputs_length = tf.compat.v1.placeholder(
          tf.int32,
          shape=[batch_size] + list(self._config.data_converter.length_shape))
      self._max_length = tf.compat.v1.placeholder(tf.int32, shape=())
      # Outputs
      ...
      if var_name_substitutions is not None:
        ...
        for v in tf.compat.v1.global_variables():
          ...
          if var_name != v.name[:-2]:
            tf.compat.v1.logging.info('Renaming `%s` to `%s`.', v.name[:-2], var_name)
          var_map[var_name] = v

      # Restore graph
      self._sess = tf.compat.v1.Session(target=session_target)
      saver = tf.compat.v1.train.Saver(var_map)
      if (os.path.exists(checkpoint_path) and
          tarfile.is_tarfile(checkpoint_path)):
        tf.compat.v1.logging.info('Unbundling checkpoint.')
