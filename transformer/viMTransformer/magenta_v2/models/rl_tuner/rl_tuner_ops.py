  ...
  try:
    ...
    for i in range(len(softmax_vect)):
      if upto + softmax_vect[i] >= r:
        return i
      upto += softmax_vect[i]
    tf.compat.v1.logging.warn("Error! sample softmax function shouldn't get here")
...
def get_variable_names(graph, scope):
  ...
  with graph.as_default():
    return [v.name for v in tf.compat.v1.global_variables() if v.name.startswith(scope)]
...
def log_sum_exp(xs):
  ...
  maxes = tf.reduce_max(input_tensor=xs, keepdims=True)
  xs -= maxes
  return tf.squeeze(maxes, [-1]) + tf.math.log(tf.reduce_sum(input_tensor=tf.exp(xs), axis=-1))
