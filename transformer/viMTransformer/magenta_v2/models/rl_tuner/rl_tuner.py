...
class RLTuner(object):
  ...
  def __init__(self, output_dir,
    ...
    with self.graph.as_default():
      ...
      if priming_mode == 'single_midi' and midi_primer is None:
        tf.compat.v1.logging.fatal('A midi primer file is required when using'
  ...
  def initialize_internal_models_graph_session(self,
    ...
    with self.graph.as_default():
      ...
      tf.compat.v1.logging.info('Initializing q network')
      ...
      tf.compat.v1.logging.info('Initializing target q network')
      ...
      tf.compat.v1.logging.info('Initializing reward network')
      ...
      tf.compat.v1.logging.info('Q network cell: %s', self.q_network.cell)

      # Add rest of variables to graph.
      tf.compat.v1.logging.info('Adding RL graph variables')
      self.build_graph()
      ...
      if restore_from_checkpoint:
        ...
        if np.sum((q1 - reward1)**2) == 0.0:
          ...
          tf.compat.v1.logging.fatal('Error! The model was not initialized from '
                           'checkpoint properly')
      else:
    ...
    if self.priming_mode == 'random_midi':
      tf.compat.v1.logging.info('Getting priming melodies')
  ...
  def get_priming_melodies(self):
    ...
    tf.compat.v1.logging.info('Stored priming notes: %s', self.priming_notes)
  ...
  def prime_internal_model(self, model):
    ...
    if self.priming_mode == 'random_midi':
      ...
      tf.compat.v1.logging.warn('Error! Invalid priming mode. Priming with random note')
      next_obs = self.get_random_note()
  ...
  def build_graph(self):
    ...
    tf.compat.v1.logging.info('Adding reward computation portion of the graph')
    with tf.compat.v1.name_scope('reward_computation'):
      self.reward_scores = tf.identity(self.reward_rnn(), name='reward_scores')

    tf.compat.v1.logging.info('Adding taking action portion of graph')
    with tf.compat.v1.name_scope('taking_action'):
      ...
      if self.algorithm == 'g':
        ...
        self.action_softmax = tf.nn.softmax(self.g_action_scores,
                                            name='action_softmax')
        self.predicted_actions = tf.one_hot(tf.argmax(input=self.g_action_scores,
                                                      axis=1,
        ...
        self.action_softmax = tf.nn.softmax(self.action_scores,
                                            name='action_softmax')
        self.predicted_actions = tf.one_hot(tf.argmax(input=self.action_scores,
                                                      axis=1,
                                                      name='predicted_actions'),
                                            self.num_actions)

    tf.compat.v1.logging.info('Add estimating future rewards portion of graph')
    with tf.compat.v1.name_scope('estimating_future_rewards'):
      ...
      if self.algorithm == 'psi':
        self.target_vals = tf.reduce_logsumexp(input_tensor=self.next_action_scores,
                                               axis=[1,])
      elif self.algorithm == 'g':
        self.g_normalizer = tf.reduce_logsumexp(input_tensor=self.reward_scores,
                                                axis=[1,])
        ...
        self.target_vals = tf.reduce_logsumexp(input_tensor=self.g_action_scores,
                                               axis=[1,])
      else:
        # Use default based on Q learning.
        self.target_vals = tf.reduce_max(input_tensor=self.next_action_scores,
                                         axis=[1,])
    ...
    tf.compat.v1.logging.info('Adding q value prediction portion of graph')
    with tf.compat.v1.name_scope('q_value_prediction'):
      # Action mask will be a one-hot encoding of the action the network
      # actually took.
      self.action_mask = tf.compat.v1.placeholder(tf.float32, (None, self.num_actions),
                                        name='action_mask')
      self.masked_action_scores = tf.reduce_sum(input_tensor=self.action_scores *
                                                self.action_mask,
                                                axis=[1,])
      ...
      # receive.
      self.prediction_error = tf.reduce_mean(input_tensor=tf.square(temp_diff))
      ...
      for grad, var in self.gradients:
        ...
        if grad is not None:
          tf.compat.v1.summary.histogram(var.name + '/gradients', grad)

      # Backprop.
      self.train_op = self.optimizer.apply_gradients(self.gradients)

    tf.compat.v1.logging.info('Adding target network update portion of graph')
    with tf.compat.v1.name_scope('target_network_update'):
    ...
    self.summarize = tf.compat.v1.summary.merge_all()
  ...
  def train(self, num_steps=10000, exploration_period=5000, enable_random=True):
    ...
    Args:
      ...
      enable_random: If False, the model will not be able to act randomly /
        explore.
    """
    tf.compat.v1.logging.info('Evaluating initial model...')
    self.evaluate_model()

    self.actions_executed_so_far = 0

    if self.stochastic_observations:
      tf.compat.v1.logging.info('Using stochastic environment')
    ...
    for i in range(num_steps):
      ...
      if i > 0 and i % self.output_every_nth == 0:
        tf.compat.v1.logging.info('Evaluating model...')
        self.evaluate_model()
        ...
        tf.compat.v1.logging.info('Training iteration %s', i)
        tf.compat.v1.logging.info('\tReward for last %s steps: %s',
                        self.output_every_nth, r)
        tf.compat.v1.logging.info('\t\tMusic theory reward: %s',
                        self.music_theory_reward_last_n)
        tf.compat.v1.logging.info('\t\tNote RNN reward: %s', self.note_rnn_reward_last_n)
        ...
        if self.exploration_mode == 'egreedy':
          ...
          tf.compat.v1.logging.info('\tExploration probability is %s', exploration_p)
  ...
  def collect_reward(self, obs, action, reward_scores):
    ...
    if self.reward_mode == 'scale':
      ...
      tf.compat.v1.logging.debug('Note RNN reward: %s', note_rnn_reward)

      reward = self.reward_music_theory(action)

      tf.compat.v1.logging.debug('Total music theory reward: %s',
                       self.reward_scaler * reward)
      tf.compat.v1.logging.debug('Total note rnn reward: %s', note_rnn_reward)
      ...
      tf.compat.v1.logging.fatal('ERROR! Not a valid reward mode. Cannot compute reward')
  ...
  def reward_music_theory(self, action):
    ...
    """
    reward = self.reward_key(action)
    tf.compat.v1.logging.debug('Key: %s', reward)
    prev_reward = reward

    reward += self.reward_tonic(action)
    if reward != prev_reward:
      tf.compat.v1.logging.debug('Tonic: %s', reward)
    prev_reward = reward

    reward += self.reward_penalize_repeating(action)
    if reward != prev_reward:
      tf.compat.v1.logging.debug('Penalize repeating: %s', reward)
    prev_reward = reward

    reward += self.reward_penalize_autocorrelation(action)
    if reward != prev_reward:
      tf.compat.v1.logging.debug('Penalize autocorr: %s', reward)
    prev_reward = reward

    reward += self.reward_motif(action)
    if reward != prev_reward:
      tf.compat.v1.logging.debug('Reward motif: %s', reward)
    prev_reward = reward

    reward += self.reward_repeated_motif(action)
    if reward != prev_reward:
      tf.compat.v1.logging.debug('Reward repeated motif: %s', reward)
    prev_reward = reward
    ...
    reward += self.reward_preferred_intervals(action)
    if reward != prev_reward:
      tf.compat.v1.logging.debug('Reward preferred_intervals: %s', reward)
    prev_reward = reward

    reward += self.reward_leap_up_back(action)
    if reward != prev_reward:
      tf.compat.v1.logging.debug('Reward leap up back: %s', reward)
    prev_reward = reward

    reward += self.reward_high_low_unique(action)
    if reward != prev_reward:
      tf.compat.v1.logging.debug('Reward high low unique: %s', reward)
  ...
  def detect_sequential_interval(self, action, key=None):
    ...
    if prev_note in (NOTE_OFF, NO_EVENT):
      tf.compat.v1.logging.debug('Action_note: %s, prev_note: %s', action_note, prev_note)
  ...
  def reward_preferred_intervals(self, action, scaler=5.0, key=None):
    ...
    tf.compat.v1.logging.debug('Interval:', interval)

    if interval == 0:  # either no interval or involving uninteresting rests
      tf.compat.v1.logging.debug('No interval or uninteresting.')
      return 0.0
    ...
    if interval == rl_tuner_ops.REST_INTERVAL:
      reward = 0.05
      tf.compat.v1.logging.debug('Rest interval.')
    if interval == rl_tuner_ops.HOLD_INTERVAL:
      reward = 0.075
    if interval == rl_tuner_ops.REST_INTERVAL_AFTER_THIRD_OR_FIFTH:
      reward = 0.15
      tf.compat.v1.logging.debug('Rest interval after 1st or 5th.')
  ...
  def detect_leap_up_back(self, action, steps_between_leaps=6):
    ...
    if action_note in (NOTE_OFF, NO_EVENT):
      ...
      tf.compat.v1.logging.debug('Rest, adding to steps since last leap. It is'
    ...
    if interval >= rl_tuner_ops.FIFTH or interval == rl_tuner_ops.IN_KEY_FIFTH:
      if action_note > prev_note:
        ...
        tf.compat.v1.logging.debug('Detected an ascending leap')
      else:
        leap_direction = rl_tuner_ops.DESCENDING
        tf.compat.v1.logging.debug('Detected a descending leap')

      # there was already an unresolved leap
      if self.composition_direction != 0:
        if self.composition_direction != leap_direction:
          tf.compat.v1.logging.debug('Detected a resolved leap')
          tf.compat.v1.logging.debug('Num steps since last leap: %s',
                           self.steps_since_last_leap)
          if self.steps_since_last_leap > steps_between_leaps:
            outcome = rl_tuner_ops.LEAP_RESOLVED
            tf.compat.v1.logging.debug('Sufficient steps before leap resolved, '
                             'awarding bonus')
          self.composition_direction = 0
          self.leapt_from = None
        else:
          tf.compat.v1.logging.debug('Detected a double leap')
          outcome = rl_tuner_ops.LEAP_DOUBLED

      # the composition had no previous leaps
      else:
        tf.compat.v1.logging.debug('There was no previous leap direction')
    ...
    # there is no leap
      ...
      self.steps_since_last_leap += 1
      tf.compat.v1.logging.debug('No leap, adding to steps since last leap. '
      ...
      if (self.composition_direction == rl_tuner_ops.ASCENDING and
        ...
        tf.compat.v1.logging.debug('detected a gradually resolved leap')
        outcome = rl_tuner_ops.LEAP_RESOLVED
        self.composition_direction = 0
        self.leapt_from = None
  ...
  def reward_leap_up_back(self, action, resolving_leap_bonus=5.0,
    ...
    if leap_outcome == rl_tuner_ops.LEAP_RESOLVED:
      tf.compat.v1.logging.debug('Leap resolved, awarding %s', resolving_leap_bonus)
      return resolving_leap_bonus
    elif leap_outcome == rl_tuner_ops.LEAP_DOUBLED:
      tf.compat.v1.logging.debug('Leap doubled, awarding %s', leaping_twice_punishment)
  ...
  def generate_music_sequence(self, title='rltuner_sample',
    ...
    tf.compat.v1.logging.info('Priming with note %s', np.argmax(next_obs))
    ...
    tf.compat.v1.logging.info('Generated sequence: %s', generated_seq)
    # TODO(natashamjaques): Remove print statement once tf.logging outputs
    # to Jupyter notebooks (once the following issue is resolved:
    # https://github.com/tensorflow/tensorflow/issues/3047)
    ...
    tf.compat.v1.logging.info('Wrote a melody to %s', self.output_dir)

    if visualize_probs:
      tf.compat.v1.logging.info('Visualizing note selection probabilities:')
      plt.figure()
  ...
  def restore_from_directory(self, directory=None, checkpoint_name=None,
    ...
    if checkpoint_name is not None:
      ...
      tf.compat.v1.logging.info('Directory %s.', directory)
      checkpoint_file = tf.train.latest_checkpoint(directory)

    if checkpoint_file is None:
      tf.compat.v1.logging.fatal('Error! Cannot locate checkpoint in the directory')
      return
    # TODO(natashamjaques): Remove print statement once tf.logging outputs
    # to Jupyter notebooks (once the following issue is resolved:
    # https://github.com/tensorflow/tensorflow/issues/3047)
    print('Attempting to restore from checkpoint', checkpoint_file)
    tf.compat.v1.logging.info('Attempting to restore from checkpoint %s', checkpoint_file)
    ...
    if reward_file_name is not None:
      ...
      tf.compat.v1.logging.info('Attempting to load saved reward values from file %s',
                      npz_file_name)
