...
class NoteRNNLoader(object):
  ...
  def __init__(self, graph, scope, checkpoint_dir, checkpoint_file=None,
    ...
    if hparams is not None:
      tf.compat.v1.logging.info('Using custom hparams')
      self.hparams = hparams
    else:
      tf.compat.v1.logging.info('Empty hparams string. Using defaults')
  ...
  def build_graph(self):
    ...
    tf.compat.v1.logging.info('Initializing melody RNN graph for scope %s', self.scope)
  ...
  def restore_vars_from_checkpoint(self, checkpoint_dir):
    ...
    Args:
      checkpoint_dir: Directory which contains a saved checkpoint of the
        model.
    """
    tf.compat.v1.logging.info('Restoring variables from checkpoint')

    var_dict = self.get_variable_name_dict()
    with self.graph.as_default():
      saver = tf.compat.v1.train.Saver(var_list=var_dict)

    tf.compat.v1.logging.info('Checkpoint dir: %s', checkpoint_dir)
    checkpoint_file = tf.train.latest_checkpoint(checkpoint_dir)
    if checkpoint_file is None:
      tf.compat.v1.logging.warn("Can't find checkpoint file, using %s",
                      self.checkpoint_file)
      checkpoint_file = self.checkpoint_file
    tf.compat.v1.logging.info('Checkpoint file: %s', checkpoint_file)
  ...
  def load_primer(self):
    ...
    if not os.path.exists(self.midi_primer):
      tf.compat.v1.logging.warn('ERROR! No such primer file exists! %s', self.midi_primer)
      return
  ...
  def prime_model(self):
    ...
    with self.graph.as_default():
      tf.compat.v1.logging.debug('Priming the model with MIDI file %s', self.midi_primer)
  ...
  def run_training_batch(self):
    ...
    if self.training_file_list is None:
      tf.compat.v1.logging.warn('No training file path was provided, cannot run training'
                      'batch')
      return

    coord = tf.train.Coordinator()
    tf.compat.v1.train.start_queue_runners(sess=self.session, coord=coord)
  ...
  def variables(self):
    ...
    """
    with self.graph.as_default():
      return [v for v in tf.compat.v1.global_variables() if v.name.startswith(self.scope)]
