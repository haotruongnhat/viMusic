...
def scaled_dot_product(Wq, Wk, Wv):
    pass
...
class RelativeGlobalAttention(keras.layers.Layer):
    ...
    def call(self, inputs, mask=None, **kwargs):
        ...
        q = inputs[0] 
        q = self.Wq(q) # [batch, num layer weight of Wq, embbeding (d)]. Example: [2, 2, 9]
        q = tf.reshape(q, (q.shape[0], q.shape[1], self.h, -1)) # Example: [2, 2, 1, 9]; h: num words
        q = tf.transpose(q, (0, 2, 1, 3))  # batch, h, seq, dh [2,1,2,9]
        '''
            h: num of word
            seq: layer weight of W
            dh: embbeding dim of word
        '''
        k = inputs[1]
        k = self.Wk(k)
        k = tf.reshape(k, (k.shape[0], k.shape[1], self.h, -1))
        k = tf.transpose(k, (0, 2, 1, 3))  # Example: [2,1,4,9]

        v = inputs[2]
        v = self.Wv(v)
        v = tf.reshape(v, (v.shape[0], v.shape[1], self.h, -1))
        v = tf.transpose(v, (0, 2, 1, 3)) # Example: [2,1,4,9]
        # set_trace()
    ...
    def _segment_level_recurrence(self, hr_pre, hr1_pre, Wq, Wk, Wv):
        new_hr1 = tf.concat([K.stop_gradient(hr_pre), hr1_pre], axis = 0)
        q = tf.matmul(hr1_pre, Wq, transpose_b = True) 
        k = tf.matmul(new_hr1, Wk, transpose_b = True)
        v = tf.matmul(new_hr1, Wv, transpose_b = True)
        # Note: Have not successful implement yet
        return None
...
if __name__ == '__main__':
    ...
    # from pdb import set_trace
    # # set_trace()
    # x = tf.random.uniform([4, 10, 256]) # random sequence with batch 4, 10 word, 256 embedding dim : [4, 10, 256]
    # viEnconder = Encoder(num_layers = 6, d_model=256, input_vocab_size=10, max_len=2048)
