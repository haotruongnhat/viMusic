...
"""
from music_transformer.music import MusicXMLDocument, MusicXMLParseError
...
def musicxml_to_sequence_proto(musicxml_document):
    ...
    """
    sequence = vimusic_pb2.NoteSequence()

    # Standard MusicXML fields.
    sequence.source_info.source_type = (
        vimusic_pb2.NoteSequence.SourceInfo.SCORE_BASED)
    sequence.source_info.encoding_type = (
        vimusic_pb2.NoteSequence.SourceInfo.MUSIC_XML)
    sequence.source_info.parser = (
        vimusic_pb2.NoteSequence.SourceInfo.MAGENTA_MUSIC_XML)
        ...
        for musicxml_measure in musicxml_part.measures:
            for musicxml_note in musicxml_measure.notes:
                if not musicxml_note.is_rest:
                    ...
                    note.text = musicxml_note.text
                    note.syllabic = musicxml_note.syllabic
