...
class MusicXMLDocument(object):
    ...
    def __init__(self, filename):
        #get score as [xml.etree.ElementTree]
...
class Note(object):
    ...
    def _parse_syllable(self,xml_lyric):
        """Parse the MusicXML <pitch> element."""
        """Get attrib"""
        attrib_dict = xml_lyric.attrib
        if len(attrib_dict) == 0:
            self.syllabic = xml_lyric.find('syllabic').text
            syllable = xml_lyric.find("text").text
            self.text = ''.join([i for i in syllable if i.isalpha()]).lower()
        elif "number" in attrib_dict.keys():
            if attrib_dict["number"] == "1":
                self.syllabic = xml_lyric.find('syllabic').text
                syllable = xml_lyric.find("text").text
                self.text = ''.join([i for i in syllable if i.isalpha()]).lower()
            else:
                #Ignore the second lyric and onward
                pass
